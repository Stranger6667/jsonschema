...
static VALIDATOR_PAIRS: &[ValidatorPair] = &[
    ...
    ValidatorPair {
        ...
        codegen_validate: Draft202012PrefixItemsValidator::validate,
    ...
    ValidatorPair {
        ...
        codegen_validate: Draft201909DependentSchemasValidator::validate,
...
fuzz_target!(|data: &[u8]| {
    ...
    for pair in VALIDATOR_PAIRS {
        ...
        let codegen_validate = (pair.codegen_validate)(&instance);
        let dynamic_validate = pair.dynamic.validate(&instance);
        assert_eq!(
            codegen_validate.is_ok(),
            dynamic_validate.is_ok(),
            "validate ok/err mismatch for schema `{}` on instance {instance}",
            pair.name
        );
        if let (Err(codegen_error), Err(dynamic_error)) = (&codegen_validate, &dynamic_validate) {
            assert_eq!(
                codegen_error.instance_path().as_str(),
                dynamic_error.instance_path().as_str(),
                "validate instance_path mismatch for schema `{}` on instance {instance}",
                pair.name
            );
        }
    }
