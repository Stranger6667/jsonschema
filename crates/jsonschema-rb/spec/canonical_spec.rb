...
RSpec.describe "JSONSchema canonical" do
  describe "JSONSchema.canonicalize" do
    ...
    it "inspect is a bounded summary" do
      schema = JSONSchema.canonicalize({ "type" => "string" })
      expect(schema.inspect).to eq("#<JSONSchema::Canonical::CanonicalSchema kind=string draft=Draft202012>")
    end
  ...
  describe "#view" do
    ...
    it "EnumView exposes its values" do
      expect(JSONSchema.canonicalize({ "enum" => [1, "two"] }).view.values).to eq(["two", 1])
    end
  end
  ...
  describe "#view string bounds" do
    it "exposes min_length, patterns, format, and extended_regex?" do
      ...
      expect(view.patterns).to eq(["^[a-z]+$"])
  ...
  describe "#view MultiTypeView" do
    it "types returns symbols" do
      ...
      expect(view.types).to match_array(%i[string integer])
    end
  end
  ...
  describe "Canonical::JSON.to_string" do
    ...
    it "normalizes a huge BigDecimal past the expansion cap to scientific form" do
      require "bigdecimal"
      result = JSONSchema::Canonical::JSON.to_string(BigDecimal("1E+1048577"))
      expect(result).to eq("1e1048577")
    end

    it "normalizes a huge Integer past the expansion cap to scientific form" do
      result = JSONSchema::Canonical::JSON.to_string(10**1_048_577)
      expect(result).to eq("1e1048577")
    end

    it "normalizes Float exponent spellings to the canonical decimal form" do
