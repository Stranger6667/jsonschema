...
RSpec.describe JSONSchema do
  ...
  describe "JSONSchema::Canonical::JSON.to_string" do
    it "sorts object keys recursively" do
      value = { "b" => 1, "a" => { "z" => 2, "y" => 3 } }
      expect(JSONSchema::Canonical::JSON.to_string(value)).to eq('{"a":{"y":3,"z":2},"b":1}')
    end

    it "normalizes integer-valued floats and special floats" do
      value = [1.0, 1.5, Float::NAN, Float::INFINITY, -Float::INFINITY]
      expect(JSONSchema::Canonical::JSON.to_string(value)).to eq("[1,1.5,null,null,null]")
    end

    it "normalizes negative zero" do
      expect(JSONSchema::Canonical::JSON.to_string(-0.0)).to eq("0")
    end

    it "serializes large integer-valued floats as integers" do
      value = 1e300
      expect(JSONSchema::Canonical::JSON.to_string(value)).to eq(value.to_i.to_s)
    end
    ...
    it "is not affected by Float#to_i monkeypatching" do
      ...
      begin
        ...
        expect(JSONSchema::Canonical::JSON.to_string(1.0)).to eq("1")
      ensure
    ...
    it "supports BigDecimal values" do
      require "bigdecimal"
      expect(JSONSchema::Canonical::JSON.to_string(BigDecimal("1.0"))).to eq("1")
      expect(JSONSchema::Canonical::JSON.to_string(BigDecimal("1E-2"))).to eq("0.01")
      expect(JSONSchema::Canonical::JSON.to_string(BigDecimal("NaN"))).to eq("null")
    end
    ...
    it "is stable for equivalent schemas" do
      ...
      expect(JSONSchema::Canonical::JSON.to_string(schema_a)).to eq(JSONSchema::Canonical::JSON.to_string(schema_b))
    end

    it "normalizes symbol and string keys the same way" do
      expect(JSONSchema::Canonical::JSON.to_string({ type: "string", properties: { name: { type: "string" } } }))
        .to eq(JSONSchema::Canonical::JSON.to_string({ "type" => "string", "properties" => { "name" => { "type" => "string" } } }))
    end

    it "raises for unsupported types" do
      expect { JSONSchema::Canonical::JSON.to_string(Object.new) }.to raise_error(TypeError, /Unsupported type/)
    end

    it "raises TypeError for invalid hash key type" do
      expect { JSONSchema::Canonical::JSON.to_string({ 1 => "value" }) }
    ...
    it "raises EncodingError for non-UTF-8 hash keys" do
      ...
      expect { JSONSchema::Canonical::JSON.to_string({ key => 1 }) }
  ...
  describe ".validator_cls_for" do
    ...
    [true, false, nil, 42, []].each do |input|
      it "defaults to Draft202012Validator for #{input.inspect}" do
        expect(JSONSchema.validator_cls_for(input)).to eq(JSONSchema::Draft202012Validator)
      end
    end
    ...
    it "detects draft from a JSON string without $schema" do
      expect(JSONSchema.validator_cls_for('{"type":"string"}')).to eq(JSONSchema::Draft202012Validator)
    end

    {
      ...
      it "returns #{expected_cls} for JSON string with $schema: #{schema_uri.inspect}" do
        schema_str = "{\"$schema\":\"#{schema_uri}\"}"
        expect(JSONSchema.validator_cls_for(schema_str)).to eq(expected_cls)
      end
    end

    it "raises ArgumentError for invalid JSON string" do
      expect { JSONSchema.validator_cls_for("not valid json") }
