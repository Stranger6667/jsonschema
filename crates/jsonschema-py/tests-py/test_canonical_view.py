...
def test_const_decimal_preserves_precision():
    schema = jsonschema_rs.canonicalize({"const": Decimal("1.0000000000000000001")})
    assert schema.to_json_schema() == {
        "$schema": "https://json-schema.org/draft/2020-12/schema",
        "const": Decimal("1.0000000000000000001"),
    }


def test_const_view_decimal_preserves_precision():
    view = jsonschema_rs.canonicalize({"const": Decimal("1.0000000000000000001")}).view()
    assert isinstance(view, jsonschema_rs.canonical.ConstView)
    assert view.value == Decimal("1.0000000000000000001")


def test_enum_view_decimal_preserves_precision():
    view = jsonschema_rs.canonicalize({"enum": [Decimal("1.0000000000000000001"), 1.5]}).view()
    assert isinstance(view, jsonschema_rs.canonical.EnumView)
    assert view.values == [Decimal("1.0000000000000000001"), 1.5]


def test_content_schema_decimal_preserves_precision():
    view = jsonschema_rs.canonicalize(
        {
            "type": "string",
            "contentMediaType": "application/json",
            "contentSchema": {"const": Decimal("1.0000000000000000001")},
        }
    ).view()
    assert isinstance(view, jsonschema_rs.canonical.StringView)
    [facet] = view.content
    assert facet.content_schema == {"const": Decimal("1.0000000000000000001")}


def test_raw_view_decimal_preserves_precision():
    view = jsonschema_rs.canonicalize({"$dynamicAnchor": "a", "const": Decimal("1.0000000000000000001")}).view()
    assert isinstance(view, jsonschema_rs.canonical.RawView)
    assert view.schema == {"$dynamicAnchor": "a", "const": Decimal("1.0000000000000000001")}
...
def test_canonical_view_type_matches_view_instances():
    from jsonschema_rs.canonical import CanonicalViewType

    view = jsonschema_rs.canonicalize({"type": "string"}).view()
    assert isinstance(view, CanonicalViewType)


def test_repr_is_bounded_summary():
    schema = jsonschema_rs.canonicalize({"type": "string"})
    assert repr(schema) == "<CanonicalSchema kind=string draft=Draft202012>"
