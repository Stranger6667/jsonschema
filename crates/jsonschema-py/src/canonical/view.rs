...
use crate::{canonical::PyCanonicalSchema, value_to_python};
...
fn number_to_py(py: Python<'_>, number: Option<Number>) -> PyResult<Option<Py<PyAny>>> {
    match number {
        Some(number) => Ok(Some(value_to_python(py, &Value::Number(number))?)),
        None => Ok(None),
    }
}

fn required_number_to_py(py: Python<'_>, number: Number) -> PyResult<Py<PyAny>> {
    value_to_python(py, &Value::Number(number))
}

fn numbers_to_py(py: Python<'_>, numbers: Vec<Number>) -> PyResult<Vec<Py<PyAny>>> {
    numbers
        .into_iter()
        .map(|number| value_to_python(py, &Value::Number(number)))
        .collect()
}
...
fn content_facet_to_py(
    ...
    let content_schema = match facet.content_schema {
        Some(value) => Some(value_to_python(py, &value)?),
        None => None,
    };
    Ok(Py::new(
        py,
...
#[pyclass(frozen, name = "IfThenElseView")]
pub(crate) struct IfThenElseViewPy {
    #[pyo3(get)]
    condition: Py<PyAny>,
    #[pyo3(get)]
    then_branch: Option<Py<PyAny>>,
    #[pyo3(get)]
    else_branch: Option<Py<PyAny>>,
}

#[pymethods]
impl IfThenElseViewPy {
    #[classattr]
    fn __match_args__() -> (&'static str, &'static str, &'static str) {
...
pub(crate) fn view_to_py(py: Python<'_>, view: CanonicalView) -> PyResult<Py<PyAny>> {
    let object = match view {
        ...
        .into_any(),
        CanonicalView::IfThenElse(v) => Py::new(
            py,
            IfThenElseViewPy {
                condition: schema_to_py(py, v.condition)?,
                then_branch: match v.then_branch {
                    Some(c) => Some(schema_to_py(py, c)?),
                    None => None,
                },
                else_branch: match v.else_branch {
                    Some(c) => Some(schema_to_py(py, c)?),
                    None => None,
                },
            },
