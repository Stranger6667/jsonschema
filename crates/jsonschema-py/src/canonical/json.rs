use jsonschema::canonical::json::NumberToken;
...
/// Decimal string specials are ascii words after the sign: NaN / sNaN / Infinity.
#[inline]
fn decimal_is_special(bytes: &[u8]) -> bool {
    ...
    S: Serializer,
{
    ...
    // Specials -> null; any other decimal text is a valid JSON-number token, normalized through the
    // core canonical number form so every surface spells one value one way.
    let result = if decimal_is_special(slice.as_bytes()) {
        serializer.serialize_unit()
    } else {
        match jsonschema::canonical::json::canonical_number(slice) {
            Some(text) => serializer.serialize_some(&NumberToken(&text)),
            None => serializer.serialize_some(&NumberToken(slice)),
        }
    };
    unsafe { ffi::Py_DECREF(string_object) };
    result
