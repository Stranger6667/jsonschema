...
#[test_case(serde_json::json!(9007199254740992.0), false ; "below_minimum_decimal_form")]
#[test_case(serde_json::json!(9007199254740994.0), true ; "above_minimum_decimal_form")]
#[test_case(serde_json::json!(9007199254740992u64), false ; "below_minimum_integer_form")]
#[test_case(serde_json::json!(9007199254740994u64), true ; "above_minimum_integer_form")]
fn test_numeric_minimum_mixed_representations_match_dynamic(
    instance: serde_json::Value,
    expected: bool,
) {
    let schema = serde_json::json!({"type":"number","minimum":9007199254740993u64});
...
fn test_base_uri_codegen_matches_dynamic(instance: serde_json::Value) {
    ...
    let dynamic = jsonschema::options()
        ...
        .with_resource(
            "json-schema:///root/defs.json",
            jsonschema::Resource::from_contents(
                serde_json::json!({"$defs":{"item":{"type":"integer","minimum":1}}}),
            ),
        )
        .build(&schema)
        .expect("valid schema");
...
fn test_dynamic_validator_ref_sibling_behavior(draft: jsonschema::Draft) {
    ...
    let validator = jsonschema::options()
        ...
        .with_resource(
            "urn:num",
            jsonschema::Resource::from_contents(serde_json::json!({"type":"number"})),
        )
        .build(&schema)
        .expect("schema should build");
...
fn build_dynamic_with_resources(
    ...
    for (uri, resource) in resources {
        options = options.with_resource(uri, jsonschema::Resource::from_contents(resource));
    }
    options
        .build(&schema)
