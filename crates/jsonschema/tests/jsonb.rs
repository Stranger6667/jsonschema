...
// `as_f64` rounds these; the literal does not.
...
// Below the smallest subnormal, a number reads back as zero without being one.
...
// `validate` reports the failing instance, which stops at `serde_json`'s parser depth.
#[test]
