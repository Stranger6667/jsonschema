...
use referencing::{
    uri, Draft, List, Resolved, Resolver, ResourceRef, Uri, Vocabulary, VocabularySet,
};
...
pub(crate) fn build_validator(
    ...
    if let Some(index) = config.index {
        return build_validator_with_index(config, schema, draft, resource_ref, index);
    }

    let (registry, _) = build_registry(config, draft, resource_ref, resource_ref.id())?;
    let index = registry.build_index()?;
    build_validator_with_index(config, schema, draft, resource_ref, &index)
}
...
pub(crate) async fn build_registry_async<'a>(
    ...
    let base_uri = resolve_base_uri(config.base_uri.as_ref(), schema_id)?;
    let registry = referencing::Registry::try_from_resources_and_retriever_async(
        once((base_uri.as_str(), resource)),
        &*config.retriever,
        draft,
    )
...
fn build_validator_with_index<R>(
    config: &ValidationOptions<'_, R>,
    schema: &Value,
    draft: Draft,
    resource_ref: ResourceRef<'_>,
    ...
    if !index.contains_resource_uri(base_uri.as_str()) {
        return Err(ValidationError::custom(format!(
            "Root URI '{base_uri}' is not present in provided index; \
             ensure the schema URI is indexed or set `with_base_uri(...)` to an indexed URI"
        )));
    }
    let vocabularies = index.find_vocabularies(draft, schema);
    let resolver = index.resolver(base_uri);
...
) -> Result<referencing::Uri<String>, referencing::Error> {
