use crate::{
    ...
    keywords::{
        ...
        format::{builtin_format, Format, StringFormat, ValueFormat},
    },
    paths::Location,
...
impl<'i, R, F: Json> ValidationOptions<'i, R, F> {
    ...
    /// ```
    #[must_use]
    pub fn with_format_value<N, Check>(mut self, name: N, format: Check) -> Self
    where
        N: Into<String>,
        Check: Fn(&Value) -> bool + Send + Sync + 'static,
    {
        self.formats
