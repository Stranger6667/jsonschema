...
/// Lightweight context for `is_valid()` - only cycle detection, no path tracking.
/// Used by the fast validation path where we only need a boolean result.
#[derive(Default)]
pub(crate) struct LightweightContext {
...
impl LightweightContext {
    ...
    /// Returns self - allows uniform code that calls `.lightweight()` on either context type.
    #[inline]
    pub(crate) fn lightweight(&mut self) -> &mut Self {
        self
    }
...
pub(crate) fn compute_final_evaluation_path(
    ...
    // Use join_raw_suffix for O(1) allocation instead of O(n) via repeated join()
    captured.eval_prefix.join_raw_suffix(suffix)
}

/// Context for `validate()`, `iter_errors()`, and `evaluate()` operations.
///
/// Tracks cycle detection during validation. Ref path tracking is now done
/// via `LazyRefPath` passed as a parameter (like `LazyLocation`).
#[derive(Default)]
pub struct ValidationContext {
    cycle: LightweightContext,
    /// Cache for formatted schema locations using ahash for fast lookups.
...
impl ValidationContext {
    ...
    #[inline]
    pub(crate) fn lightweight(&mut self) -> &mut LightweightContext {
        &mut self.cycle
    }

    #[inline]
    pub(crate) fn enter(&mut self, node_id: usize, instance: &Value) -> bool {
        self.cycle.enter(node_id, instance)
    }

    #[inline]
    pub(crate) fn exit(&mut self, node_id: usize, instance: &Value) {
        self.cycle.exit(node_id, instance);
    }
...
///
/// Cost: O(n*m) where n = stack depth, m = average path length.
/// This is only called when creating errors, so valid instances pay O(0).
#[inline]
pub(crate) fn capture_evaluation_path(
    ...
    if ref_path.is_empty() {
        ...
        let captured = CapturedRefState::from(ref_path);
...
///
/// When calling `is_valid` from within `validate`, use `ctx.lightweight()` to get the inner
/// `LightweightContext` reference.
pub(crate) trait Validate: ThreadBound {
    fn iter_errors<'i>(
        ...
        match self.validate(instance, location, ref_path, ctx) {
            Ok(()) => no_error(),
            Err(err) => error(err),
        }
    }
...
impl Validator {
    ...
    pub fn validate<'i>(&self, instance: &'i Value) -> Result<(), ValidationError<'i>> {
        ...
        let ref_path = LazyRefPath::new();
        self.root
            .validate(instance, &LazyLocation::new(), &ref_path, &mut ctx)
    }
    ...
    pub fn iter_errors<'i>(&'i self, instance: &'i Value) -> ErrorIterator<'i> {
        ...
        let ref_path = LazyRefPath::new();
        self.root
            .iter_errors(instance, &LazyLocation::new(), &ref_path, &mut ctx)
    }
    ...
    pub fn is_valid(&self, instance: &Value) -> bool {
        let mut ctx = LightweightContext::new();
    ...
    pub fn evaluate(&self, instance: &Value) -> Evaluation {
        ...
        let root = self
            .root
            .evaluate_instance(instance, &LazyLocation::new(), &ref_path, &mut ctx);
        Evaluation::new(root)
    }
