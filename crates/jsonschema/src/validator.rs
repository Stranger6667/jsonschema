...
use crate::{
    ...
    evaluation::{Evaluation, EvaluationNode},
    node::SchemaNode,
...
pub(crate) trait Validate: ThreadBound {
    ...
    /// `evaluate_instance` method which returns an [`EvaluationNode`](crate::evaluation::EvaluationNode).
...
pub(crate) enum EvaluationResult {
    Valid {
        ...
        child_results: Vec<EvaluationNode>,
    },
    Invalid {
...
impl EvaluationResult {
    ...
    pub(crate) fn annotate(&mut self, new_annotations: Annotations) {
        match self {
            ...
            Self::Invalid { annotations, .. } => *annotations = Some(new_annotations),
        }
    }
    ...
    pub(crate) fn mark_errored(&mut self, error: ErrorDescription) {
        match self {
            ...
            Self::Valid {
                ...
                *self = Self::Invalid {
                    errors: vec![error],
                    child_results: std::mem::take(child_results),
