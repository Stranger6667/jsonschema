...
impl num_traits::ToPrimitive for BoundCardinality {
...
#[cfg(not(feature = "arbitrary-precision"))]
impl From<BoundCardinality> for serde_json::Number {
    fn from(value: BoundCardinality) -> Self {
...
mod tests {
    ...
    #[test_case(5 => (Some(5i64), Some(5u64), Some(5i128)) ; "five")]
    fn to_primitive(value: u64) -> (Option<i64>, Option<u64>, Option<i128>) {
        let value = cardinality(value);
        (
            ToPrimitive::to_i64(&value),
            ToPrimitive::to_u64(&value),
            ToPrimitive::to_i128(&value),
        )
    }
    ...
    // A count above `i64::MAX` is representable as `u64`/`BigInt` but not as `i64`.
    #[test]
    fn to_primitive_above_i64_range() {
        let value = BoundCardinality::from(u64::MAX);
        assert_eq!(ToPrimitive::to_i64(&value), None);
        assert_eq!(ToPrimitive::to_u64(&value), Some(u64::MAX));
    }
