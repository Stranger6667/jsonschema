...
// No default-build `From<u64>`: values past `i64::MAX` have no exact form there, and a lossy
// conversion silently corrupts bounds; fallible call sites convert via `i64::try_from`.
#[cfg(feature = "arbitrary-precision")]
...
impl BoundInteger {
    ...
    // Inherent mirrors of `num_traits`/`num_integer` so callers need no trait imports.
    ...
    // The default build converts spans via `i128`; only `arbitrary-precision` narrows to `u64` here.
    #[cfg(feature = "arbitrary-precision")]
    ...
    pub(crate) fn div_ceil(&self, other: &Self) -> Self {
        Self(num_integer::Integer::div_ceil(&self.0, &other.0))
    }
...
mod tests {
    ...
    fn gcd(left: i64, right: i64) -> BoundInteger {
        integer(left).gcd(&integer(right))
    }

    #[test_case(17, 5 => (integer(3), integer(2)) ; "17 div 5")]
    fn div_rem(left: i64, right: i64) -> (BoundInteger, BoundInteger) {
        integer(left).div_rem(&integer(right))
    }

    #[test_case(17, 5 => integer(4) ; "rounds up")]
    #[test_case(15, 5 => integer(3) ; "exact")]
    #[test_case(-17, 5 => integer(-3) ; "negative rounds toward zero")]
    fn div_ceil(left: i64, right: i64) -> BoundInteger {
        integer(left).div_ceil(&integer(right))
    }

    #[cfg(feature = "arbitrary-precision")]
    #[test_case(5u64 => integer(5) ; "from u64")]
    #[test_case(u64::MAX => "18446744073709551615".parse::<BoundInteger>().expect("in range") ; "past i64")]
    ...
    // `div_rem` is truncated division; the remainder keeps the dividend's sign.
    #[test_case(-17, 5 => (integer(-3), integer(-2)) ; "negative dividend")]
    #[test_case(17, -5 => (integer(-3), integer(2)) ; "negative divisor")]
    fn div_rem_negative(left: i64, right: i64) -> (BoundInteger, BoundInteger) {
        integer(left).div_rem(&integer(right))
    }
