...
impl CanonicalJson {
    ...
    /// (`1.0` -> `"1"`), so the `.`/`e`/`E` byte-scan is exact; past-cap values stay scientific (scan as `Number`).
    #[must_use]
...
impl StringLeaf {
    /// Whether any positive or negative pattern needs the extended (fancy) regex engine. Derived on demand,
    /// not stored, so it never drifts across the pattern edits intersection/negation perform.
    #[must_use]
    pub(crate) fn extended_regex(&self) -> bool {
        self.patterns
            .iter()
            .chain(&self.not_patterns)
            .any(|pattern| pattern_is_extended(pattern))
    }
}

/// True for patterns the standard `regex` crate can't compile (lookaround, backreferences). Patterns that fail
/// ECMA translation are flagged extended; the real compile error surfaces later when the validator runs.
fn pattern_is_extended(pattern: &str) -> bool {
    match crate::ecma::to_rust_regex(pattern) {
        Ok(translated) => regex::Regex::new(&translated).is_err(),
        Err(()) => true,
    }
}
...
fn is_full_boolean_enum(values: &[CanonicalJson]) -> bool {
    values.len() == 2
        && values
            .iter()
            .map(CanonicalJson::as_str)
            .all(|s| matches!(s, "true" | "false"))
        && values[0].as_str() != values[1].as_str()
}

fn is_null_boolean_enum(values: &[CanonicalJson]) -> bool {
    if values.len() != 3 {
        return false;
    }
    let mut bits: u8 = 0;
...
pub(crate) struct SchemaNode {
    ...
    /// Structural hash of `schema`, cached at construction. Ordering and equality compare it first so they
    /// short-circuit instead of walking the whole subtree; a deep compare only runs on a hash tie.
    pub(crate) hash: u64,
...
impl Schema {
    ...
    pub(crate) fn finite_values_saturate_type(values: &[CanonicalJson]) -> Option<JsonType> {
        match values {
            [only] if only.as_str() == "null" => Some(JsonType::Null),
            [first, second]
                if (first.as_str() == "false" && second.as_str() == "true")
                    || (first.as_str() == "true" && second.as_str() == "false") =>
            {
    ...
    pub(crate) fn as_type_set(&self) -> Option<JsonTypeSet> {
        ...
        match self {
            // `null` is the only inhabitant of the null type.
            Self::Const(value) if value.as_str() == "null" => {
                Some(JsonTypeSet::from(JsonType::Null))
            }
            Self::Enum(values) if is_full_boolean_enum(values) => {
                Some(JsonTypeSet::from(JsonType::Boolean))
            }
            Self::Enum(values) if is_null_boolean_enum(values) => {
                Some(JsonTypeSet::from(JsonType::Null).insert(JsonType::Boolean))
            }
