...
use crate::canonical::ir::{Schema, SchemaKindSet, SchemaNode, SharedSchema};
...
pub(crate) fn shared(schema: Schema) -> SharedSchema {
    let mask = schema.variant_bit().union(child_mask(&schema));
    let hash = structural_hash(&schema);
    let size = tree_size(&schema);
    Arc::new(SchemaNode {
        schema,
        mask,
        hash,
...
fn tree_size(schema: &Schema) -> u32 {
    let mut size: u32 = 1;
...
fn child_mask(schema: &Schema) -> SchemaKindSet {
    let mut mask = SchemaKindSet::empty();
    schema.for_each_child(|child| {
