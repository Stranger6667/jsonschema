...
// Two-branch `AllOf` fallback when a precise intersection can't be computed. Every intersect bail
// must go through here with the untouched operands: `is_residual_pair` recognizes exactly this
// shape, and a bail it misses gets committed as a reduction and can re-trigger folds every sweep.
#[must_use]
