...
use crate::{
    canonical::{
        ...
        ir::{IntegerLeaf, Schema, SharedSchema},
...
/// ```
pub(super) fn fold_type_guard_any_of(branches: &mut Vec<SharedSchema>, draft: Draft) -> bool {
    if branches.len() != 2 {
        return false;
    }
    for complement_index in 0..branches.len() {
        ...
        if !guard_spelling_is_canonical(ty, &branches[body_index], draft) {
            continue;
        }
...
/// ```
pub(super) fn fold_guard_conjunction_any_of(
    branches: &mut Vec<SharedSchema>,
    draft: Draft,
) -> bool {
    if branches.len() < 2 {
    ...
    for (index, branch) in branches.iter().enumerate() {
        ...
        if !guard_spelling_is_canonical(slot, &view.schema, draft) {
            return false;
        }
...
fn guard_spelling_is_canonical(ty: JsonType, body: &SharedSchema, draft: Draft) -> bool {
    if ty == JsonType::Number && matches!(body.as_schema(), Schema::Const(_)) {
        return true;
    }
    negate_type_guard(ty, body, draft).is_some()
}
...
pub(super) fn merge_dual_facet_leaves(branches: &mut Vec<SharedSchema>, draft: Draft) -> bool {
    for left in 0..branches.len() {
        ...
        let Some(negated) = negate_type_guard(guard_kind, &branches[left], draft) else {
            continue;
        };
        let merged = open_typed_leaf(guard_kind);
...
pub(super) fn covers_every_json_type(branches: &[SharedSchema]) -> bool {
    let covered = branches.iter().fold(JsonTypeSet::empty(), |acc, branch| {
        match branch.as_schema().single_unconstrained_type() {
            Some(ty) => acc.insert(ty),
            None => acc,
        }
    });
    covered == JsonTypeSet::all()
}
