...
pub(super) fn drop_contains_upper_bound_covered_by_sibling(
    ...
    for index in 0..branches.len() {
        ...
        delta.contains.sort();
        let delta = shared(Schema::Array(delta));
        let weakened = shared(Schema::Array(weakened));
...
/// the wide overlapping spelling is canonical. Array analog of [`drop_min_properties_covered_by_sibling`].
///
/// ```text
