...
/// ```
/// Cap on raising a repeated-items floor past sibling-covered lengths. Coverage runs can extend as
/// far as the widest sibling window, so termination needs a cap; an unsettled walk reverts (sound -
/// only convergence strength is lost).
const MIN_ITEMS_RAISE_CAP: u8 = 8;
...
pub(super) fn raise_array_min_items_covered_by_sibling(
    ...
    for index in 0..branches.len() {
        ...
        for _ in 0..MIN_ITEMS_RAISE_CAP {
