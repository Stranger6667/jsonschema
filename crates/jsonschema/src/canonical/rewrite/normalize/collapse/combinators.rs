...
use crate::canonical::{
    ...
    intern::shared,
    intersect::{
        intersect_canonical, is_unmergeable_string_pair, is_unsafe_numeric_pair,
        object::is_unsafe_object_pair,
    },
...
fn try_pairwise_intersect_capped<O>(
    ...
    for outer_idx in 0..branches.len() {
        ...
        for inner_idx in inner_start..branches.len() {
            ...
            if is_unsafe_object_pair(&left, &right, ctx) || is_unsafe_numeric_pair(&left, &right) {
                continue;
            }
