...
/// Recurse into each branch, drop the combinator's identity element, splice nested same-variant
/// combinators, and report whether anything changed.
fn collapse_branches(
    branches: &[SharedSchema],
    ctx: &CanonicalizationContext,
    is_identity: fn(&Schema) -> bool,
    splice: fn(&Schema) -> Option<&[SharedSchema]>,
) -> (Vec<SharedSchema>, bool) {
    let mut changed = false;
    ...
    for branch in branches {
        ...
        if is_identity(next.as_schema()) {
            changed = true;
            continue;
        }
        if let Some(inner) = splice(next.as_schema()) {
            changed = true;
            cleaned.extend(inner.iter().cloned());
            continue;
        }
        cleaned.push(next);
    }
...
/// Zero branches collapse to the combinator's empty-case constant, one unwraps; otherwise sort,
/// dedup, and rebuild only when something changed.
fn rebuild_combinator(
    mut cleaned: Vec<SharedSchema>,
    mut changed: bool,
    schema: &SharedSchema,
    empty: fn() -> Schema,
    wrap: fn(Vec<SharedSchema>) -> Schema,
) -> SharedSchema {
    match cleaned.len() {
        0 => shared(empty()),
        ...
        _ => {
            ...
            match cleaned.len() {
                1 => cleaned.into_iter().next().expect("len == 1"),
                _ if changed => shared(wrap(cleaned)),
                _ => Arc::clone(schema),
            }
        }
    }
...
fn collapse_all_of(
    ...
    let (mut cleaned, mut changed) = collapse_branches(
        branches,
        ctx,
        |schema| matches!(schema, Schema::True),
        |schema| match schema {
            Schema::AllOf(inner) => Some(inner.as_slice()),
            _ => None,
        },
    );
    if cleaned
    ...
    changed |= numeric::simplify_intersection_branches(&mut cleaned, ctx.draft());
    ...
    rebuild_combinator(cleaned, changed, schema, || Schema::True, Schema::AllOf)
}
...
fn collapse_any_of(
    ...
    let (mut cleaned, mut changed) = collapse_branches(
        branches,
        ctx,
        |schema| matches!(schema, Schema::False),
        |schema| match schema {
            Schema::AnyOf(inner) => Some(inner.as_slice()),
            _ => None,
        },
    );
    if cleaned
    ...
    changed |= merge_dual_facet_leaves(&mut cleaned, ctx.draft());
    ...
    changed |= drop_string_pattern_facet_covered_by_sibling(&mut cleaned, ctx);
    ...
    changed |= fold_type_guard_any_of(&mut cleaned, ctx.draft());
    changed |= fold_guard_conjunction_any_of(&mut cleaned, ctx.draft());
    rebuild_combinator(cleaned, changed, schema, || Schema::False, Schema::AnyOf)
}
