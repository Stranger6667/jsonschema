...
impl super::NormalizeStage for ObjectStage {
    ...
    fn rewrite(recursed: SharedSchema, _ctx: &CanonicalizationContext) -> SharedSchema {
        let Schema::Object(original_leaf) = recursed.as_schema() else {
            return recursed;
        };
        let zero_max_normalized = drop_constraints_under_zero_max(original_leaf);
        let original_leaf = zero_max_normalized.as_ref().unwrap_or(original_leaf);
        let requirement_normalized = normalize_requirements(original_leaf);
        let normalized_leaf = requirement_normalized.as_ref().unwrap_or(original_leaf);
        // Close `required` under dependentRequired so the collapse below sees the final required-name set.
        let folded = fold_required_closure(normalized_leaf);
        let folded_leaf = folded.as_ref().unwrap_or(normalized_leaf);
        let default_normalized = drop_true_object_defaults(folded_leaf);
        let leaf: &ObjectLeaf = default_normalized.as_ref().unwrap_or(folded_leaf);
        if !additional_properties_is_false(leaf) {
            ...
            let next_leaf = trimmed
                .or(default_normalized)
                .or(folded)
                .or(requirement_normalized)
                .or_else(|| zero_max_normalized.clone());
            return match next_leaf {
                Some(leaf) => shared(Schema::Object(leaf)),
                None => recursed,
            };
        }
        if has_allowing_pattern(leaf) {
            return match default_normalized
                .or(folded)
                .or(requirement_normalized)
                .or_else(|| zero_max_normalized.clone())
            {
