...
const LINEAR_ENCODING_THRESHOLD: usize = 7;
...
fn canonicalize_impl(schema: &SharedSchema, ctx: &CanonicalizationContext) -> SharedSchema {
    ...
    // Cache each branch's recursive descent so the naive encoding is O(N).
    let processed: Vec<SharedSchema> = branches.iter().map(|b| canonicalize(b, ctx)).collect();
    ...
    if processed.len() < LINEAR_ENCODING_THRESHOLD {
        naive_encode(&processed)
    } else {
...
fn naive_encode(processed: &[SharedSchema]) -> SharedSchema {
    let rewritten: Vec<SharedSchema> = (0..processed.len())
        .map(|pivot| {
            let mut conjunction = Vec::with_capacity(processed.len());
            conjunction.push(Arc::clone(&processed[pivot]));
            for (index, other) in processed.iter().enumerate() {
                if index == pivot {
                    continue;
                }
                conjunction.push(shared(Schema::Not(Arc::clone(other))));
            }
            shared(Schema::AllOf(conjunction))
        })
        .collect();
    shared(Schema::AnyOf(rewritten))
}

/// Encodes "exactly one branch matches" as a balanced reduction tree, replacing [`naive_encode`]'s
/// O(N²) pairwise expansion with an O(N) one.
///
