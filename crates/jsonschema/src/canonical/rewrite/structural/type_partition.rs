...
use crate::{
    canonical::{
        ...
        intersect::{
            ...
            has_unsafe_numeric_pair, intersect_internal, is_residual_pair,
            object::has_unsafe_object_pair,
        },
...
fn partition_all_of(
    ...
    // Pre-filters skipping folds known to hit a residual (the per-step check in
    // `fold_with_intersect` owns correctness; these only avoid wasted attempts).
    ...
    {
        if let Some(folded) = fold_with_intersect(&flattened, ctx) {
            return folded;
        }
    }
...
/// Fold with the non-canonicalising intersect, canonicalize once at the end; `None` when a step
/// does not reduce. Intersection is associative and canonicalisation confluent, so the result
/// matches per-step canonicalisation without re-running the pipeline each step.
fn fold_with_intersect(
    branches: &[SharedSchema],
    ctx: &CanonicalizationContext,
) -> Option<SharedSchema> {
    let mut iterator = branches.iter();
    let Some(first) = iterator.next() else {
        return Some(shared(Schema::True));
    };
    let mut accumulator = Arc::clone(first);
    for branch in iterator {
        let raw = intersect_internal(&accumulator, branch, ctx);
        // A step the leaf algebra does not reduce means the fold would commit a residual whose
        // canonicalized form re-enters this stage; keep the conjunction as it stands instead.
        if is_residual_pair(&raw, &accumulator, branch) {
            return None;
        }
        accumulator = raw;
    ...
    Some(canonicalize_ir(&accumulator, ctx))
}
