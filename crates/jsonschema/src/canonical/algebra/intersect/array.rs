use crate::canonical::{
    ...
    prover::Prover,
    rewrite::normalize::collapse::drop_strictly_dominated,
};
...
/// ```
///
/// Windows must match and the wider clause must be unbounded above; a finite `maxContains` bounds a larger
/// count the stricter clause cannot imply.
fn drop_subsumed_contains(clauses: &mut Vec<ContainsClause>, ctx: &CanonicalizationContext) {
    drop_strictly_dominated(clauses, |wider, stricter| {
        is_subsumed_by(wider, stricter, ctx)
    });
}
...
fn is_subsumed_by(
    ...
    // A finite `maxContains` on the wider clause bounds a larger match count than the subset stricter clause
    // caps, so it is not redundant.
    if wider.max_contains.is_some() {
        return false;
    }
