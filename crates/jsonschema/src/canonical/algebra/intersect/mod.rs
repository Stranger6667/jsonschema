...
/// Whether `result` is the residual `AllOf` of exactly the untouched operands - the leaf algebra's
/// "does not reduce" verdict.
pub(crate) fn is_residual_pair(
    result: &SharedSchema,
    left: &SharedSchema,
    right: &SharedSchema,
) -> bool {
    let Schema::AllOf(branches) = result.as_schema() else {
        return false;
    };
    // `SchemaNode::eq` pointer-fast-paths shared allocations before comparing structurally.
    matches!(
        branches.as_slice(),
        [first, second]
...
/// The canonicalized intersection, or `None` when the pair does not reduce (the algebra bailed to
/// the residual `AllOf` of the untouched operands). Folds must never commit a non-reduction: its
/// canonicalized form can restructure and re-trigger the fold on every sweep.
pub(crate) fn intersect_reduced(
    left: &SharedSchema,
    right: &SharedSchema,
    ctx: &CanonicalizationContext,
) -> Option<SharedSchema> {
    ...
    let raw = intersect_internal(left, right, ctx);
    if is_residual_pair(&raw, left, right) {
        return None;
    }
    Some(canonicalize_ir(&raw, ctx))
}
