...
impl Leaf for ObjectLeaf {
    ...
    fn intersect(&self, other: &Self, ctx: &CanonicalizationContext) -> Intersection<Self> {
        ...
        // Catch-all plus pattern can't flatten into one object; keep both as AllOf.
...
fn object_name_rejected(schema: &SharedSchema, name: &str) -> bool {
    match schema.as_schema() {
        ...
        Schema::String(string_leaf) => {
            let length = u64::try_from(name.chars().count()).unwrap_or(u64::MAX);
...
// True when `left` and `right` are both objects whose intersection is unsafe to flatten (see `unsafe_to_flatten`).
pub(crate) fn is_unsafe_object_pair(left: &SharedSchema, right: &SharedSchema) -> bool {
    matches!(
        (left.as_schema(), right.as_schema()),
        (Schema::Object(left), Schema::Object(right)) if unsafe_to_flatten(left, right)
    )
}

// True when any pair of branches is an unsafe object pair (its intersection bails back to `AllOf`).
pub(crate) fn has_unsafe_object_pair(branches: &[SharedSchema]) -> bool {
    any_unordered_pair(branches, is_unsafe_object_pair)
}
...
fn intersect_constraints(
    ...
    for constraint in left {
        ...
        if let PropertyNameMatcher::NamedProperty(name) = &constraint.matcher {
            // The catch-all governs only names matched by nothing on the opposite side - an exact name that also
            // matches an opposite pattern is exempt.
            if let Some(catch_all) = right_catch_all {
                let is_right_exact = right_exact_names
                    .as_ref()
                    .is_some_and(|names| names.contains(name));
                let matches_right_pattern =
                    right_patterns.iter().any(|(regex, _)| regex.is_match(name));
                if !is_right_exact && !matches_right_pattern {
                    schema = intersect_internal(&schema, catch_all, ctx);
                }
            }
            for (regex, pattern_schema) in &right_patterns {
    ...
    for constraint in right {
        ...
        let mut schema = Arc::clone(&constraint.schema);
        if let PropertyNameMatcher::NamedProperty(name) = &constraint.matcher {
            // This loop only sees right names with no left matcher, so `name` is never a left exact name; the catch-all
            // is exempt only when matched by a left pattern.
            if let Some(catch_all) = left_catch_all {
                let matches_left_pattern =
                    left_patterns.iter().any(|(regex, _)| regex.is_match(name));
                if !matches_left_pattern {
                    schema = intersect_internal(&schema, catch_all, ctx);
                }
            }
            for (regex, pattern_schema) in &left_patterns {
                if regex.is_match(name) {
                    schema = intersect_internal(&schema, pattern_schema, ctx);
                }
            }
