...
pub(crate) fn intersect_multi_type_with_value_set(
    ...
    let Some(values) = value_set.as_schema().finite_values() else {
        return SharedSchema::clone(value_set);
    };
...
pub(crate) fn is_unmergeable_guard_value_pair(
    ...
    let Some(values) = other.as_schema().finite_values() else {
        return false;
    };
...
fn typed_with_value_set<'a>(
    ...
    let values = value_set.as_schema().finite_values()?;
