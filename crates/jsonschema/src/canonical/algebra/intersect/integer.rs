...
use crate::canonical::{
    ...
    ir::{BoundInteger, IntegerBounds, IntegerLeaf, Schema, SharedSchema},
...
/// True when intersecting the two integer leaves would yield an unrepresentable combined `multipleOf` and therefore
/// bail back to `AllOf`.
pub(crate) fn is_unsafe_integer_pair(left: &SharedSchema, right: &SharedSchema) -> bool {
    let (Schema::Integer(left), Schema::Integer(right)) = (left.as_schema(), right.as_schema())
    else {
        return false;
    };
