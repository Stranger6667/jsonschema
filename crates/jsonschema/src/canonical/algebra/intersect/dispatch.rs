...
use crate::{
    canonical::{
        ...
        ir::{
            ArrayLeaf, BooleanBounds, CanonicalJson, IntegerLeaf, NumberLeaf, ObjectLeaf, Schema,
...
use super::{
    ...
    value_set::{
        filter_values_through_guard, guarded_value_set, intersect_multi_type_with_value_set,
    },
};
...
pub(crate) fn intersect_internal(
    ...
    match (left.as_schema(), right.as_schema()) {
        ...
        (Schema::MultiType(set), Schema::Const(_) | Schema::Enum(_)) => {
            intersect_multi_type_with_value_set(*set, right, ctx)
        }
        (Schema::Const(_) | Schema::Enum(_), Schema::MultiType(set)) => {
            intersect_multi_type_with_value_set(*set, left, ctx)
        }
...
fn intersect_type_guard_with_schema(
    ...
    if let Some(values) = other.as_schema().finite_values() {
        return match filter_values_through_guard(guard_ty, guard_body, values, ctx) {
            Some(kept) => guarded_value_set(guard_ty, guard_body, kept, ctx),
...
fn lift_to_ty(target_ty: JsonType, body_ty: JsonType, body: &SharedSchema) -> SharedSchema {
    ...
    match (target_ty, body_ty, body.as_schema()) {
        (JsonType::Integer, JsonType::Number, Schema::Number(number)) => {
            // `not_multiple_of` holds finite positive moduli, so the integer conversion never fails in practice;
            // the fallback keeps the number leaf rather than guess on an unrepresentable modulus or bound.
            ...
            let Some(bounds) = number_bounds_to_integer(&number.bounds) else {
                return Arc::clone(body);
            };
            let Some(multiple_of) = number_multiple_of_to_integer(number.multiple_of.as_ref())
            else {
                return Arc::clone(body);
            };
            let leaf = IntegerLeaf {
                bounds,
                multiple_of: multiple_of.into_modulus(),
                not_multiple_of,
            };
        ...
        // A value-set body keeps only members of the target type; the rest lie outside the narrowed kind.
        (JsonType::Integer, JsonType::Number, Schema::Const(value)) => {
            if value.json_type() == JsonType::Integer {
                Arc::clone(body)
            } else {
                shared(Schema::False)
            }
        }
        (JsonType::Integer, JsonType::Number, Schema::Enum(values)) => {
            let kept: Vec<CanonicalJson> = values
                .iter()
                .filter(|value| value.json_type() == JsonType::Integer)
                .cloned()
                .collect();
            intern_value_set(kept)
        }
        _ => {
            if target_ty.covers(body_ty) {
                // Widening: the body is already confined to a subtype of the target.
                Arc::clone(body)
            } else {
                // Any other narrowing pins the body under the target type instead of dropping the pin.
...
/// Re-wrap a merged body under the `TypedGroup` pin an operand carried; bare body otherwise. A body that
/// already carries the same pin (the value-set path finalizes with one) is returned as-is.
fn typed_group_if(wrap: bool, ty: JsonType, body: SharedSchema) -> SharedSchema {
    if !wrap
        || matches!(body.as_schema(), Schema::TypedGroup { ty: existing, .. } if *existing == ty)
    {
        body
    } else {
...
mod tests {
    ...
    use crate::{canonical::tests_util::canonicalize_with, Draft};

    // The Draft4 integer pin is applied once: a value-set merge that already produced a
    // `TypedGroup` must not be wrapped in a second one.
    #[test]
    fn draft4_value_set_merge_wraps_typed_group_once() {
        let left = canonicalize_with(&json!({"type": "integer", "enum": [1, 2]}), Draft::Draft4);
        let right = canonicalize_with(&json!({"type": "integer", "minimum": 0}), Draft::Draft4);
        assert_eq!(
            left.intersect(&right).to_json_schema(),
            json!({
                "$schema": "http://json-schema.org/draft-04/schema#",
                "allOf": [{"type": "integer"}, {"enum": [1, 2]}]
            })
        );
    }
}
