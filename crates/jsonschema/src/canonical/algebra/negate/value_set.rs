...
/// AFTER:  {"not": {"const": "x"}}
/// ```
pub(super) fn negate_const(value: &CanonicalJson, draft: Draft) -> SharedSchema {
    ...
    match &raw {
        ...
        Value::Number(number) => {
            if number.is_i64() || number.is_u64() {
                // Draft4 const equality is numeric but its "integer" type is token-based, so the
                // integer decomposition (whose number slot is "number and not integer") would admit
                // the `5.0` spelling the const also matches; only `not(const)` is the exact complement.
                if draft == Draft::Draft4 {
                    return not_wrap(shared(Schema::Const(value.clone())));
                }
...
///         ]}
/// ```
pub(super) fn negate_enum(values: &[CanonicalJson], draft: Draft) -> SharedSchema {
    if values.is_empty() {
        return shared(Schema::True);
    }
    let conjuncts: Vec<SharedSchema> = values
        .iter()
        .map(|value| negate_const(value, draft))
        .collect();
...
mod tests {
    ...
    // Draft4 const/enum equality is numeric while its "integer" type is token-based, so the integer
    // decomposition would admit the 5.0 spelling the const also matches.
    #[test]
    fn draft4_integer_const_negation_rejects_float_spelling() {
        let negated = canonicalize_with(&json!({"enum": [5]}), Draft::Draft4)
            .negate()
            .to_json_schema();
        let validator = crate::options()
            .with_draft(Draft::Draft4)
            .build(&negated)
            .expect("negation compiles");
        assert!(!validator.is_valid(&json!(5)));
        assert!(!validator.is_valid(&json!(5.0)));
