...
/// (`not multipleOf 1`), which the integer-kind complement alone misses. Under Draft 4 the integer body is
/// lexical while `not multipleOf 1` is value-based (it excludes `2.0`), so the Integer dual stays symbolic.
    ...
    let branches = match body.as_schema() {
        ...
        Schema::Integer(leaf)
            if !keeps_draft4_integer_guard(JsonType::Integer, draft)
                && !leaf.bounds.dual_is_unrepresentable() =>
        {
...
/// is the `q = 1` spelling (`multipleOf: 1` normalizes to it). Under Draft 4 an integer body is
/// lexical, not the `q = 1` spelling, so it has no `multipleOf` dual.
pub(super) fn negate_number_multiple_of_type_guard(
    ...
    match body.as_schema() {
        ...
        Schema::Integer(leaf)
            if !keeps_draft4_integer_guard(JsonType::Integer, draft)
                && leaf.bounds == IntegerBounds::default()
...
mod tests {
    ...
    fn integer_body_number_guard_dual_adds_non_integers() {
        ...
        let dual = super::super::negate_type_guard(
            JsonType::Number,
            &body,
            referencing::Draft::Draft202012,
        )
        .expect("numeric guard dual");
