...
mod tests {
    ...
    use crate::canonical::{ir::Schema, tests_util::canonicalize};
    ...
    #[test]
    fn negate_additional_properties_is_closed() {
        // additionalProperties:{integer} accepts {} and {"a":1}; rejects an object with a non-integer additional prop.
        ...
        for value in [json!({}), json!({"a": 1})] {
            assert!(pos.is_valid(&value));
            assert!(!neg.is_valid(&value));
        }
        for value in [json!({"a": "x"}), json!(0)] {
            assert!(!pos.is_valid(&value));
            assert!(neg.is_valid(&value));
        }
    }
    ...
    #[test]
    fn negate_additional_properties_false_with_properties_is_exact() {
        // Declared `properties` scope the additional set: the negated existential must not count `a` as additional.
        let schema = json!({"type": "object", "properties": {"a": {"type": "integer"}}, "additionalProperties": false});
        let pos = crate::validator_for(&schema).expect("positive compiles");
        let negated = canonicalize(&schema).negate().to_json_schema();
        let neg = crate::validator_for(&negated).expect("negated compiles");
        for value in [json!({}), json!({"a": 1})] {
        ...
        for value in [json!({"a": "x"}), json!({"b": 2}), json!(5)] {
            assert!(!pos.is_valid(&value));
            assert!(neg.is_valid(&value));
        }
    }
