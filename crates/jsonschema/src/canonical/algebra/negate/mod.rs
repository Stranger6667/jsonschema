...
pub(crate) fn negate_for_draft(
    ...
    negate_with_options(
        schema,
        NegationOptions {
            use_contains_for_array_tail: draft.is_known_keyword("contains"),
        },
        ctx,
    )
}
...
fn negate_inner(
    ...
    match schema.as_schema() {
        ...
        Schema::TypeGuard { ty: kind, body } => {
            negate_type_guard(*kind, body).unwrap_or_else(|| {
        ...
        Schema::Const(value) => negate_const(value),
        Schema::Enum(values) => negate_enum(values),
...
pub(crate) fn negate_type_guard(kind: JsonType, body: &SharedSchema) -> Option<SharedSchema> {
    negate_string_pattern_type_guard(kind, body)
        ...
        .or_else(|| negate_number_multiple_of_type_guard(kind, body))
        .or_else(|| negate_numeric_bounds_type_guard(kind, body))
}
...
mod tests {
    ...
    use crate::{
        canonical::{
            ...
            tests_util::{canonicalize, cardinality, const_json},
            CanonicalSchema,
        },
        JsonType,
    };
    ...
    // Negate a JSON schema through the pipeline and assert exact set complement: rejects the original's accepts,
    // accepts its rejects.
    fn assert_json_complement(schema: &Value, accepts: &[Value], rejects: &[Value]) {
