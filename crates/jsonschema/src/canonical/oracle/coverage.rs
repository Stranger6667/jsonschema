...
use crate::{
    canonical::{
        ...
        ir::{
            ArrayLeaf, BoundCardinality, BoundFraction, Bounds, CanonicalJson, ContainsClause,
            IntegerBounds, IntegerLeaf, LengthBounds, NumberBounds, NumberLeaf, ObjectConstraint,
            ObjectLeaf, ObjectRequirement, PropertyNameMatcher, Schema, SharedSchema, StringLeaf,
        },
        leaves::{
...
pub(crate) fn any_sibling_covers(
    ...
    branches
        .iter()
        .enumerate()
        .any(|(sibling, branch)| !skip.contains(&sibling) && covers_with(&prover, branch, delta))
}
...
fn covers_within_query(prover: &Prover<'_>, big: &SharedSchema, small: &SharedSchema) -> bool {
    ...
    if !prover.has_definitions() {
        // Pointer keys make hits allocation-free; each entry retains its `Arc`s so the addresses
        // stay valid for the memo's lifetime.
        let key = (Arc::as_ptr(big), Arc::as_ptr(small));
        if let Some(cached) = prover
            .ctx()
            .covers_memo()
            .borrow()
            .get(&key)
        ...
        if result || prover.ctx().partition_fuel_remaining() > 0 {
            prover
                ...
                .covers_memo()
                .borrow_mut()
                .insert(key, (Arc::clone(big), Arc::clone(small), result));
        }
        return result;
    }
...
fn covers_uncached(prover: &Prover<'_>, big: &SharedSchema, small: &SharedSchema) -> bool {
    ...
    match (big.as_schema(), small.as_schema()) {
        ...
        (_, Schema::AllOf(branches)) => {
            ...
            if let Some(facets) = leaf_facets(big.as_schema(), prover.ctx()) {
        ...
        (_, Schema::MultiType(small_set)) => {
            if let Some(big_set) = big.as_schema().as_type_set() {
                return Schema::type_set_covers(big_set, *small_set);
            }
            // A type set over `null`/`boolean` is a finite domain: covered iff `big` admits every member.
            let Some(domain) = null_boolean_domain(*small_set) else {
                return false;
            };
            domain
                .iter()
...
/// The values inhabiting a type set whose member types are all finite (`null`/`boolean`); `None` otherwise.
fn null_boolean_domain(set: JsonTypeSet) -> Option<Vec<CanonicalJson>> {
    let mut values = Vec::new();
    for ty in set {
        match ty {
            JsonType::Null => values.push(CanonicalJson::from_value(&Value::Null)),
            JsonType::Boolean => {
                values.push(CanonicalJson::from_value(&Value::Bool(false)));
                values.push(CanonicalJson::from_value(&Value::Bool(true)));
            }
            _ => return None,
        }
    }
...
/// `leaf = ⋂ facets` exactly, one single-constraint leaf per facet; `None` for kinds without a
/// decomposition (object requirement/constraint pairs interact and cannot split field-by-field)
/// and for leaves with fewer than two constraints, where the decomposition is the leaf itself.
/// Facets are interned so repeated queries hit the pointer-keyed covers memo instead of
/// accumulating dead entries under fresh addresses.
fn leaf_facets(schema: &Schema, ctx: &CanonicalizationContext) -> Option<Vec<SharedSchema>> {
    let facets = match schema {
        Schema::String(leaf) => string_leaf_facets(leaf),
        Schema::Integer(leaf) => numeric_leaf_facets(leaf),
        Schema::Number(leaf) => numeric_leaf_facets(leaf),
        Schema::Array(leaf) => array_leaf_facets(leaf),
        _ => return None,
    };
    if facets.len() < 2 {
        return None;
    }
    Some(facets.into_iter().map(|facet| ctx.intern(facet)).collect())
}

fn numeric_leaf_facets<L: NumericLeaf>(leaf: &L) -> Vec<SharedSchema> {
    let bounds = leaf.bounds();
    let count = usize::from(bounds.minimum.is_some())
        + usize::from(bounds.maximum.is_some())
        + usize::from(leaf.multiple_of().is_some())
        + leaf.not_multiple_of().len();
    if count < 2 {
        return Vec::new();
    }
    let mut facets = Vec::with_capacity(count);
    if bounds.minimum.is_some() {
        facets.push(shared(L::into_schema(
            Bounds {
                minimum: bounds.minimum.clone(),
    ...
    debug_assert_eq!(facets.len(), count);
    facets
}

/// `prefix`/`tail` interact (the tail constrains items past the prefix), so they stay one facet.
fn array_leaf_facets(leaf: &ArrayLeaf) -> Vec<SharedSchema> {
    let open = ArrayLeaf::default();
    let count = usize::from(!leaf.prefix.is_empty() || leaf.tail != open.tail)
        + usize::from(!leaf.length.minimum.is_zero())
        + usize::from(leaf.length.maximum.is_some())
        + usize::from(leaf.unique_items)
        + usize::from(leaf.repeated_items)
        + leaf.contains.len();
    if count < 2 {
        return Vec::new();
    }
    let mut facets = Vec::with_capacity(count);
    if !leaf.prefix.is_empty() || leaf.tail != open.tail {
        facets.push(shared(Schema::Array(ArrayLeaf {
            prefix: leaf.prefix.clone(),
            tail: Arc::clone(&leaf.tail),
            ..ArrayLeaf::default()
        })));
    }
    if !leaf.length.minimum.is_zero() {
        facets.push(shared(Schema::Array(ArrayLeaf {
            length: LengthBounds {
    ...
    if leaf.length.maximum.is_some() {
        facets.push(shared(Schema::Array(ArrayLeaf {
            length: LengthBounds {
...
fn string_leaf_facets(leaf: &StringLeaf) -> Vec<SharedSchema> {
    let count = usize::from(leaf.min_length.is_some())
        + usize::from(leaf.max_length.is_some())
        + leaf.patterns.len()
        + leaf.not_patterns.len()
        + usize::from(leaf.format.is_some())
        + leaf.content.len();
    if count < 2 {
        return Vec::new();
    }
...
pub(crate) fn object_leaf_covers(
    ...
    let small_by_matcher: AHashMap<&PropertyNameMatcher, &ObjectConstraint> =
        small.constraints.iter().map(|c| (&c.matcher, c)).collect();
    for big_constraint in &big.constraints {
        if small_admits_no_properties || matches!(big_constraint.schema.as_schema(), Schema::True) {
            continue;
        }
        let Some(small_constraint) = small_by_matcher.get(&big_constraint.matcher).copied() else {
...
pub(crate) fn array_leaf_covers(big: &ArrayLeaf, small: &ArrayLeaf, prover: &Prover<'_>) -> bool {
    ...
    // A `small` clause over the same item schema counts the same items, so a window inside the
    // `big` clause's window implies it.
    let clause_implied_by_small = |clause: &ContainsClause| {
        small.contains.iter().any(|small_clause| {
            small_clause.schema == clause.schema
                && small_clause.min_contains >= clause.min_contains
                && match (
                    clause.max_contains.as_ref(),
                    small_clause.max_contains.as_ref(),
                ) {
                    (None, _) => true,
                    (Some(_), None) => false,
                    (Some(big_max), Some(small_max)) => small_max <= big_max,
                }
        })
    };
    if !big
        .contains
        .iter()
        .all(|clause| clause_vacuous(clause) || clause_implied_by_small(clause))
    {
        return false;
    }
...
pub(crate) fn string_leaf_covers(
    big: &StringLeaf,
    small: &StringLeaf,
    ctx: &CanonicalizationContext,
) -> bool {
    ...
    if big.extended_regex(ctx) != small.extended_regex(ctx) {
        return false;
    }
...
mod tests {
    ...
    #[test_case(
        json!({"type": "array", "contains": {"type": "integer"}, "minContains": 1, "maxContains": 1, "maxItems": 2}),
        json!({"type": "array", "contains": {"type": "integer"}, "minContains": 0, "maxContains": 1, "maxItems": 2}),
        Some(true)
        ; "same_schema_contains_window_containment")]
    #[test_case(
        json!({"type": "array", "contains": {"type": "integer"}, "minContains": 1, "maxContains": 2, "maxItems": 2}),
        json!({"type": "array", "contains": {"type": "integer"}, "minContains": 0, "maxContains": 1, "maxItems": 2}),
        None
        ; "wider_small_window_stays_inconclusive")]
    ...
    // Self-conjunction distributes over the inner union and re-reduces through the contains-window
    // implication; without it the `[1,1]`-window cross-term survives and `a ∧ a != a`.
    #[test]
    fn self_conjunction_of_contains_union_is_idempotent() {
        let x = json!({"anyOf": [
            {"type": "integer"},
            {"contains": {"type": "integer"}, "maxContains": 1, "maxItems": 2, "minContains": 0, "type": "array"},
            {"contains": {"type": "integer"}, "maxContains": 2, "maxItems": 2, "minContains": 1, "type": "array"}
        ]});
        let conjunction = canonicalize(&json!({"allOf": [x, x]})).expect("conjunction");
        let baseline = canonicalize(&x).expect("baseline");
        assert_eq!(conjunction.as_schema(), baseline.as_schema());
    }

    // `a ∨ a` fuses the negation branches into `not(allOf)`, whose reduction needs the same
    // contains-window implication.
    #[test]
    fn union_with_self_reduces_to_identity() {
        ...
        let a = canonicalize(&schema).expect("canonicalizes");
        let reduced = canonicalize(&a.union(&a).to_json_schema()).expect("reduced");
        let baseline = canonicalize(&a.to_json_schema()).expect("baseline");
        assert_eq!(reduced.as_schema(), baseline.as_schema());
    }
