...
/// # Panics
///
/// Panics if the canonical serializer emits invalid UTF-8, which it never does.
...
fn canonical_number(raw: &str) -> Option<Cow<'_, str>> {
    ...
    if !has_exponent && bytes[fraction_end - 1] != b'0' {
        return Some(Cow::Borrowed(raw));
    }
...
fn canonical_scientific_number(digits: &[u8], parts: &NumberParts<'_>) -> Option<String> {
    ...
    // Only reached from the `shift >= 0` branch (non-negative explicit exponent), so `exponent_digits` is always the non-empty digit slice.
    let mut exponent = BigInt::parse_bytes(parts.exponent_digits.as_bytes(), 10)?;
...
mod tests {
    ...
    #[test_case::test_case(&nest_arrays(300) ; "arrays")]
    #[test_case::test_case(&nest_objects(300) ; "objects")]
    ...
    mod write_f64 {
        ...
        fn formats(value: &serde_json::Value, expected: &str) {
