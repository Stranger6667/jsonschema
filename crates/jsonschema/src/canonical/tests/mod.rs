...
// Parse owns unsupported-bound routing: a bound past the carrier errors (routing the document to
// raw preservation) instead of silently dropping the constraint, even without the `document`
// pre-scan in front.
#[cfg(not(feature = "arbitrary-precision"))]
#[test]
fn parse_rejects_bound_past_the_carrier() {
    let schema = json!({"type": "array", "minItems": 1e100});
    let error = crate::canonical::parse::parse_graph(
        &schema,
        Draft::Draft202012,
        None,
        usize::MAX,
    ...
    .expect_err("bound past the carrier must error");
    assert!(
        matches!(error, CanonicalizationError::InvalidJsonValue(_)),
        "got {error:?}"
    );
}

// The draft decides `to_json_schema`'s `$schema` header and keyword semantics, and the
// format-assertion flag decides whether `format` rejects instances, so both are identity.
#[test]
fn canonical_schema_equality_and_hash_distinguish_draft_and_formats() {
    let schema = json!({"type": "integer"});
    let draft7 = options()
        .with_draft(Draft::Draft7)
        .canonicalize(&schema)
        .expect("draft7 canonicalize");
    let draft2020 = options()
        .with_draft(Draft::Draft202012)
        .canonicalize(&schema)
        .expect("draft2020 canonicalize");
    let draft7_again = options()
        .with_draft(Draft::Draft7)
        .canonicalize(&schema)
        .expect("draft7 canonicalize");

    assert_ne!(draft7, draft2020);
    assert_eq!(draft7, draft7_again);
    ...
    let asserted = options()
        ...
        .canonicalize(&email)
        .expect("asserted canonicalize");
    let annotated = options()
        .with_draft(Draft::Draft202012)
        .should_validate_formats(false)
        .canonicalize(&email)
        .expect("annotated canonicalize");
    assert_ne!(asserted, annotated);

    let mut hash_set = HashSet::new();
    hash_set.insert(draft7.clone());
    hash_set.insert(draft2020.clone());
    hash_set.insert(asserted.clone());
    hash_set.insert(annotated.clone());
    assert_eq!(hash_set.len(), 4);

    let mut tree_set = BTreeSet::new();
    tree_set.insert(draft7);
    tree_set.insert(draft2020);
    tree_set.insert(asserted);
    tree_set.insert(annotated);
