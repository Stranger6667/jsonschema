...
fn bounded_int(tc: &TestCase) -> i32 {
...
fn arbitrary_leaf_schema(tc: TestCase) -> Value {
    ...
    match choice {
        ...
        3 => json!({"type": "string"}),
        4 => {
            let (minimum, maximum) = ordered(bounded_int(&tc), bounded_int(&tc));
        ...
        19 => json!({"type": "integer", "exclusiveMinimum": bounded_int(&tc)}),
        20 => {
...
fn kind_leaf(tc: &TestCase, kind: u8) -> Value {
    ...
    match (kind, pick) {
        ...
        (0, _) => json!({"type": "integer", "exclusiveMinimum": bounded_int(tc)}),
...
fn kind_instance(tc: &TestCase, kind: u8) -> Value {
    match kind {
        0 => json!(bounded_int(tc)),
        1 => match tc.draw(gs::integers::<u8>().min_value(0).max_value(1)) {
            0 => json!(bounded_int(tc)),
            _ => json!(f64::from(bounded_int(tc)) + 0.5),
        },
        2 => {
