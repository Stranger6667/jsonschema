...
type CoversMemoKey = (*const SchemaNode, *const SchemaNode);
type CoversMemoEntry = (SharedSchema, SharedSchema, bool);
...
pub(crate) struct CanonicalizationContext {
    ...
    covers_memo: RefCell<AHashMap<CoversMemoKey, CoversMemoEntry>>,
...
impl CanonicalizationContext {
    ...
    pub(crate) fn covers_memo(&self) -> &RefCell<AHashMap<CoversMemoKey, CoversMemoEntry>> {
        &self.covers_memo
    }
    ...
    /// The canonical `Arc` for this node's structure; inserts on first sight. Same-hash collisions
    /// resolve by deep equality, so non-interned children only cost pointer-memo misses, never
    /// wrong unification.
...
pub(crate) fn pattern_is_extended_with(options: PatternEngineOptions, pattern: &str) -> bool {
    // Process-wide memo: `view()` builds a fresh context per call, so per-context caches never
    // survive between calls and every walk would re-translate and re-compile each pattern.
    // Flushed wholesale past the cap to stay bounded.
    type ExtendedPatternCache = AHashMap<PatternEngineOptions, AHashMap<Arc<str>, bool>>;
    static EXTENDED_PATTERN_CLASSIFICATIONS: LazyLock<Mutex<ExtendedPatternCache>> =
        LazyLock::new(|| Mutex::new(AHashMap::new()));
    const CACHE_CAP: usize = 4096;

    if let Some(&cached) = EXTENDED_PATTERN_CLASSIFICATIONS
        .lock()
    ...
    let result = match jsonschema_regex::to_rust_regex(pattern) {
        Ok(translated) => {
    ...
    let mut cache = EXTENDED_PATTERN_CLASSIFICATIONS
        .lock()
        .unwrap_or_else(std::sync::PoisonError::into_inner);
    let inner = cache.entry(options).or_default();
    if inner.len() >= CACHE_CAP {
        inner.clear();
    }
    inner.insert(Arc::from(pattern), result);
    result
}
