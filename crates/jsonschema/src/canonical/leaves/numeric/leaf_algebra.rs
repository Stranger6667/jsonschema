...
pub(crate) trait NumericBounds: Clone {
    ...
    /// Whether some complement half-line falls outside the bound model. JSON integers extend past the
    /// default build's `i64` carrier, so a constructive dual at its edges would silently saturate.
    fn dual_is_unrepresentable(&self) -> bool {
        false
    }
}
...
pub(crate) trait NumericLeaf: Sized {
    ...
    /// Whether `value` is a multiple of `modulus` (`modulus` is known non-zero); `None` when the
    /// divisibility test overflows the default build's magnitude carrier.
    fn value_is_multiple(value: &Self::Scalar, modulus: &Self::Modulus) -> Option<bool>;
...
impl NumericBounds for IntegerBounds {
    ...
    fn dual_is_unrepresentable(&self) -> bool {
        // Inclusive bounds need the +/-1 neighbor for the half-line; exclusive bounds reuse the
        // endpoint, but a saturated endpoint makes the half-line read as the whole carrier.
        let minimum_dual_missing = self.minimum.as_ref().is_some_and(|minimum| {
            if self.exclusive_minimum {
                minimum.checked_increment().is_none()
            } else {
                minimum.checked_decrement().is_none()
            }
        });
        let maximum_dual_missing = self.maximum.as_ref().is_some_and(|maximum| {
            if self.exclusive_maximum {
                maximum.checked_decrement().is_none()
            } else {
                maximum.checked_increment().is_none()
            }
        });
        minimum_dual_missing || maximum_dual_missing
    }

    /// Exclusive markers shift each endpoint by one before comparing; a shift with no representable
    /// neighbor bounds an empty half-line, which covers trivially or admits nothing at that end.
    fn covers(&self, other: &Self) -> bool {
        let lower_ok = match (&self.minimum, &other.minimum) {
            ...
            (Some(big_minimum), Some(small_minimum)) => {
                let big_effective = if self.exclusive_minimum {
                    big_minimum.checked_increment()
                } else {
                    Some((big_minimum).owned())
                };
                let small_effective = if other.exclusive_minimum {
                    small_minimum.checked_increment()
                } else {
                    Some((small_minimum).owned())
                };
                match (big_effective, small_effective) {
                    (_, None) => true,
                    (None, Some(_)) => false,
                    (Some(big), Some(small)) => big <= small,
                }
            }
        };
        if !lower_ok {
        ...
        match (&self.maximum, &other.maximum) {
            ...
            (Some(big_maximum), Some(small_maximum)) => {
                let big_effective = if self.exclusive_maximum {
                    big_maximum.checked_decrement()
                } else {
                    Some((big_maximum).owned())
                };
                let small_effective = if other.exclusive_maximum {
                    small_maximum.checked_decrement()
                } else {
                    Some((small_maximum).owned())
                };
                match (big_effective, small_effective) {
                    (_, None) => true,
                    (None, Some(_)) => false,
...
impl NumericLeaf for IntegerLeaf {
    ...
    fn value_is_multiple(value: &BoundInteger, modulus: &BoundInteger) -> Option<bool> {
        Some(modulus.divides(value))
    }
...
impl NumericLeaf for NumberLeaf {
    ...
    /// `small / big` is a whole number, so every `small` multiple is also a `big` multiple. Zero modulus
    /// or a ratio overflowing the default carrier declines.
    fn modulus_covers(big: Option<&BoundFraction>, small: Option<&BoundFraction>) -> bool {
        match (big, small) {
            ...
            (Some(big_modulus), Some(small_modulus)) => {
                ...
                small_modulus
                    .checked_div(big_modulus)
                    .is_some_and(|ratio| ratio.denominator_is_one())
            }
        }
    }

    fn value_is_multiple(value: &BoundFraction, modulus: &BoundFraction) -> Option<bool> {
