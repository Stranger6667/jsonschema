...
/// more keeps the range leaf, everything else is a value set (`False` when empty). `None` if a value resists
/// canonical JSON. Under Draft 4 the range leaf's lexical `type: integer` rejects fractionless floats the set's
/// members equal, so the window keeps the value-set form there.
fn emit_finite_core(sorted: &[BoundFraction], draft: Draft) -> Option<SharedSchema> {
    match classify_finite(sorted) {
        FiniteForm::Window { min, max }
            if sorted.len() >= 3 && !keeps_draft4_integer_guard(JsonType::Integer, draft) =>
        {
...
pub(crate) fn emit_finite(sorted: &[BoundFraction], draft: Draft) -> Option<Vec<SharedSchema>> {
    emit_finite_core(sorted, draft).map(|schema| match schema.as_schema() {
...
fn emit_finite_single(members: &[BoundFraction], draft: Draft) -> Option<SharedSchema> {
    Some(
        emit_finite(members, draft)?
            .pop()
...
/// set (`False` when empty). `None` outside the bounded-fractional-grid fragment. The default-build runtime
/// compares instances via a decimal derived from their f64 (absorbing ~1 ULP), which exact value-set equality
/// cannot mirror, so the respell applies only under `arbitrary-precision`.
pub(crate) fn bounded_number_grid_leaf(leaf: &NumberLeaf, draft: Draft) -> Option<SharedSchema> {
    if cfg!(not(feature = "arbitrary-precision")) {
        return None;
    }
    let members = bounded_number_grid_members(leaf)?;
    emit_finite_single(&members, draft)
}
...
pub(crate) fn bounded_integer_grid_leaf(leaf: &IntegerLeaf, draft: Draft) -> Option<SharedSchema> {
    ...
    // O(1) declines that skip candidate enumeration: a plain range of three or more members
    // re-emits itself, and a grid wider than the enumeration cap always bails.
    if leaf.not_multiple_of.is_empty() {
        let span = maximum.checked_sub(&minimum);
        if leaf.multiple_of.is_none() {
            if span
                .as_ref()
                .is_none_or(|span| span.to_i64().is_none_or(|width| width >= 2))
            {
                return None;
            }
        } else {
    ...
    let result = emit_finite_single(&members, draft)?;
