...
fn fraction_to_i64(value: &BoundFraction) -> Option<i64> {
    value
        .to_integer()
        .and_then(|integer| num_traits::ToPrimitive::to_i64(&integer))
}
...
fn enumerate_bounded_multiples(
    ...
    loop {
        let candidate = BoundFraction::from(factor.owned()) * (step).owned();
        ...
        if !exclusions
            .iter()
            .any(|exclusion| exclusion.divides(&candidate))
        {
...
pub(super) fn bounded_number_grid_members(leaf: &NumberLeaf) -> Option<Vec<BoundFraction>> {
    ...
    let first_factor = ((minimum).owned() / step.owned()).ceil_integer()?;
...
pub(crate) fn bounded_integer_grid_leaf(leaf: &IntegerLeaf, draft: Draft) -> Option<SharedSchema> {
    ...
    let step_integer = match leaf.multiple_of.as_ref() {
        Some(modulus) if num_traits::Zero::is_zero(modulus) => return None,
        Some(modulus) => num_traits::Signed::abs(modulus),
    ...
    let first_factor = num_integer::Integer::div_ceil(&minimum, &step_integer);
