...
macro_rules! div_floor_parts {
    ($numerator:expr, $denominator:expr) => {{
        let (quotient, remainder) = ::num_integer::Integer::div_rem($numerator, $denominator);
        if ::num_traits::Signed::is_negative(&remainder) {
            quotient - <$crate::canonical::ir::BoundInteger as ::num_traits::One>::one()
        } else {
            quotient
        }
    }};
...
macro_rules! div_ceil_parts {
    ($numerator:expr, $denominator:expr) => {{
        let (quotient, remainder) = ::num_integer::Integer::div_rem($numerator, $denominator);
        if ::num_traits::Zero::is_zero(&remainder) || ::num_traits::Signed::is_negative(&remainder)
        {
            quotient
        } else {
            quotient + <$crate::canonical::ir::BoundInteger as ::num_traits::One>::one()
...
impl BoundFraction {
    ...
    pub(crate) fn checked_lcm(&self, other: &BoundFraction) -> Option<BoundFraction> {
        ...
        Some(BoundFraction::from(numerator) / BoundFraction::from(denominator))
    }

    #[must_use]
    pub(crate) fn numerator_or_zero(&self) -> BoundInteger {
        self.numer()
            .map_or_else(BoundInteger::zero, |v| magnitude_to_integer!(v))
    }

    #[must_use]
    pub(crate) fn denominator_or_one(&self) -> BoundInteger {
        self.denom()
            .map_or_else(BoundInteger::one, |v| magnitude_to_integer!(v))
    }
    ...
    #[must_use]
    pub(crate) fn floor(&self) -> BoundInteger {
        let (signed_numerator, denominator) = signed_fraction_parts_lossy(self);
        div_floor_parts!(&signed_numerator, &denominator)
    }

    #[must_use]
    pub(crate) fn ceil(&self) -> BoundInteger {
        let (signed_numerator, denominator) = signed_fraction_parts_lossy(self);
        div_ceil_parts!(&signed_numerator, &denominator)
    }
    ...
    #[must_use]
    pub(crate) fn divides(&self, value: &BoundFraction) -> bool {
        if self.is_zero() {
            return false;
        }
        (value.owned() / self.owned()).denominator_is_one()
    }
}

/// Fractional values round inward (lower -> ceil, upper -> floor); fractional exclusive endpoints behave as inclusive
/// after rounding inward.
pub(crate) fn number_bounds_to_integer(bounds: &NumberBounds) -> IntegerBounds {
...
/// exclusivity; integer `v` preserves its exclusivity flag.
fn lift_bound(
    value: Option<&BoundFraction>,
    exclusive: bool,
    round: fn(&BoundFraction) -> BoundInteger,
) -> (Option<BoundInteger>, bool) {
    let Some(value) = value else {
        return (None, false);
    };
    if let Some(integer) = value.to_integer() {
        return (Some(integer), exclusive);
    }
    (Some(round(value)), false)
}

/// For `m = p/q` in lowest terms the smallest integer multiple is `p`; returns `None` when `m = 1`.
pub(crate) fn number_multiple_of_to_integer(
    multiple_of: Option<&BoundFraction>,
) -> Option<BoundInteger> {
    let value = multiple_of?;
...
fn signed_fraction_parts_lossy(value: &BoundFraction) -> (BoundInteger, BoundInteger) {
    let numerator = value.numerator_or_zero();
    let denominator = value.denominator_or_one();
    (apply_fraction_sign(value, numerator), denominator)
}
