...
use crate::{
    canonical::{
        document::{
            exceeds_depth_limit, external_target_folds_unevaluated, raw_subschema,
...
fn enter_id_scope<'a, 'r>(
    ...
    let id_keyword = if ctx.draft == Draft::Draft4 {
        "id"
    } else {
        "$id"
    };
    let id = map.get(id_keyword).and_then(Value::as_str)?;
...
fn inline_local_target(
    ...
    parse_cycle_aware(
        ...
        move |ctx| parse_inner(target, ctx),
    )
}
...
fn resolve_external(
    ...
    parse_cycle_aware(
        ...
        |_, _| {},
        move |ctx| {
            // Mirrors the root depth gate: everything below recurses over the target document.
            ...
            // The opaque-preservation check skips `$ref` targets, so an external pairing `unevaluated*` with a sibling
            // applicator would fold those keywords lossily; preserve it verbatim. Other opaque cases break if detached.
            if external_target_folds_unevaluated(contents, resolved_draft) {
                return Ok(raw_subschema(contents));
            }
...
fn build_typed_leaf(
    ...
    match kind {
        ...
        JsonType::Integer => Ok(parse_integer_leaf(map)),
        JsonType::Number => Ok(Schema::Number(NumberLeaf {
            bounds: parse_number_bounds(map),
            multiple_of: map.get("multipleOf").and_then(parse_bigfraction_value),
...
fn ensure_valid_regex(pattern: &str, pointer: &str) -> Result<(), CanonicalizationError> {
    if crate::ecma::to_rust_regex(pattern).is_err() {
...
fn parse_string_leaf(map: &Map<String, Value>) -> Result<Schema, CanonicalizationError> {
    let min_length = map
        .get("minLength")
        .and_then(parse_cardinality_bound)
    ...
    Ok(Schema::String(StringLeaf {
        ...
        min_length,
        max_length: map.get("maxLength").and_then(parse_cardinality_bound),
        patterns,
...
fn parse_array_leaf(
    ...
    let min_items = map
        .get("minItems")
        .and_then(parse_cardinality_bound)
        .unwrap_or_else(|| BoundCardinality::from(0u64));
    ...
    Ok(Schema::Array(ArrayLeaf {
        ...
        length: LengthBounds {
            minimum: min_items,
            maximum: map.get("maxItems").and_then(parse_cardinality_bound),
        },
...
fn parse_object_leaf(
    ...
    if let Some(value) = map.get("minProperties") {
        match parse_cardinality_bound(value) {
            Some(count) if !count.is_zero() => {
    ...
    if let Some(value) = map.get("maxProperties") {
        if let Some(count) = parse_cardinality_bound(value) {
...
fn parse_integer_leaf(map: &Map<String, Value>) -> Schema {
    let number_bounds = parse_number_bounds(map);
    let bounds = number_bounds_to_integer(&number_bounds);
    let multiple_of = match map.get("multipleOf").and_then(parse_bigfraction_value) {
        Some(value) => number_multiple_of_to_integer(Some(&value)),
        None => None,
    };
...
fn parse_number_bounds(map: &Map<String, Value>) -> NumberBounds {
    let mut minimum = map.get("minimum").and_then(parse_bigfraction_value);
    let mut maximum = map.get("maximum").and_then(parse_bigfraction_value);
    ...
    if let Some(value) = map
        ...
        .and_then(parse_bigfraction_value)
    {
...
fn bigfraction_from_number(number: &Number) -> Option<BoundFraction> {
    #[cfg(feature = "arbitrary-precision")]
    {
    ...
    #[cfg(not(feature = "arbitrary-precision"))]
    {
        ...
        if let Some(value) = number.as_f64() {
            return BoundFraction::try_from(value).ok();
        }
    }
