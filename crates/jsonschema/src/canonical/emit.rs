...
pub(crate) fn to_json_schema(
    ...
    let root_reference = ctx.definition_refs.get(root).cloned();
...
/// Leaf names of same-document definition keys and bare-anchor keys, reserved so synthesized names never collide.
fn existing_definition_leaf_names(definitions: &DefinitionMap) -> AHashSet<String> {
    definitions
        .keys()
        .filter_map(|uri| {
            same_document_leaf_name(uri).or_else(|| {
                strip_synthetic_root(uri)
                    .strip_prefix('#')
...
fn emit_integer(leaf: &IntegerLeaf, draft: Draft) -> Map<String, Value> {
    ...
    insert_integer_bounds(&mut map, &leaf.bounds, draft);
...
fn emit_number(leaf: &NumberLeaf, draft: Draft) -> Value {
    ...
    if leaf.bounds == NumberBounds::default() && leaf.multiple_of.is_none() {
        if let [modulus] = leaf.not_multiple_of.as_slice() {
            if non_integer_marker_needs_type_exclusion(modulus, draft) {
                return json!({"type": "number", "not": {"type": "integer"}});
            }
    ...
    let all_of: Vec<Value> = leaf
        ...
        .map(|modulus| {
            if non_integer_marker_needs_type_exclusion(modulus, draft) {
                ...
                json!({"not": {"type": "number", "multipleOf": modulus.to_json_value()}})
            }
        })
        .collect();
    if !all_of.is_empty() {
...
/// Whether this `not multipleOf` modulus is the non-integer marker (`1`) that must emit as a `type` exclusion.
/// Under Draft 4 an integer-valued float (`2.0`) is a non-integer yet a multiple of `1`, so `not multipleOf 1`
/// would wrongly admit it; `not: {type: integer}` does not.
fn non_integer_marker_needs_type_exclusion(modulus: &BoundFraction, draft: Draft) -> bool {
    draft == Draft::Draft4 && modulus.is_one()
}
    ...
    if let Some(format) = &leaf.format {
        ...
        match (ctx.validate_formats, draft_default_asserts) {
            (true, false) => {
                ...
                if let Some(assertion) = format_assertion_schema(format) {
...
fn format_assertion_schema(format: &str) -> Option<Value> {
    // Only a pivot draft whose consumers assert formats by default can carry the assertion via
    // `$schema`; without one the keyword on the parent schema is the whole spelling.
    let pivot = format_assertion_pivot_draft(format)?;
    let uri = schema_uri(pivot)?;
    let mut branch = Map::new();
    branch.insert(
        "$id".into(),
        Value::String(format!("urn:jsonschema:format-assertion:{format}")),
    );
    branch.insert("$schema".into(), Value::String(uri.to_string()));
    branch.insert("format".into(), Value::String(format.to_string()));
    Some(Value::Object(branch))
}

fn format_assertion_pivot_draft(format: &str) -> Option<Draft> {
    [Draft::Draft7, Draft::Draft6, Draft::Draft4]
        .into_iter()
        .find(|&draft| crate::keywords::format::is_known_format(draft, format))
}
...
fn emit_type_guard(kind: JsonType, body: &SharedSchema, ctx: &EmitContext) -> Value {
    match body.as_schema() {
        ...
        {
            #[rustfmt::skip]
            debug_assert!(matches!(kind, JsonType::Number | JsonType::String | JsonType::Array | JsonType::Object), "emit_without_type guard kind {kind:?} does not re-pin after stripping `type`");
...
fn insert_integer_bounds(map: &mut Map<String, Value>, bounds: &IntegerBounds, draft: Draft) {
    insert_bound(
        map,
        BoundSide::Min,
