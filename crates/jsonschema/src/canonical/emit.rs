...
fn plain_name_anchor_identifier(uri: &str, draft: Draft) -> Option<(&'static str, String)> {
    ...
    if draft.is_known_keyword("$anchor") {
        Some(("$anchor", anchor.to_string()))
    } else if draft.is_known_keyword("$id") {
        Some(("$id", format!("#{anchor}")))
    } else if draft.is_known_keyword("id") {
        Some(("id", format!("#{anchor}")))
    } else {
        None
...
fn defs_container(draft: Draft) -> &'static str {
    if draft.is_known_keyword("$defs") {
        "$defs"
    } else {
        "definitions"
    }
...
fn emit_number(leaf: &NumberLeaf, draft: Draft) -> Value {
    ...
    let all_of: Vec<Value> = leaf
        .not_multiple_of
        .iter()
        .map(|modulus| json!({"not": {"type": "number", "multipleOf": modulus.to_json_value()}}))
        .collect();
    if !all_of.is_empty() {
        map.insert("allOf".into(), Value::Array(all_of));
    }
...
fn emit_array(leaf: &ArrayLeaf, ctx: &EmitContext) -> Value {
    ...
    if leaf.prefix.is_empty() {
        ...
        let (tuple_key, tail_key) = if draft.is_known_keyword("prefixItems") {
            ("prefixItems", "items")
        } else {
...
fn emit_type_guard(kind: JsonType, body: &SharedSchema, ctx: &EmitContext) -> Value {
    match body.as_schema() {
        ...
        Schema::Integer(leaf) if kind == JsonType::Number => {
            // Draft 4 has no value-based integer spelling (`multipleOf: 1` admits floats the
            // lexical `type: integer` body rejects), so the guard keeps the generic form.
            if keeps_draft4_integer_guard(JsonType::Integer, ctx.draft) {
                json!({
                    "anyOf": [
