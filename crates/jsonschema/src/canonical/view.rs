...
impl CanonicalSchema {
    ...
    pub fn view(&self) -> CanonicalView {
        match self.as_schema() {
            ...
            Schema::String(leaf) => CanonicalView::String(string_view(leaf, &self.context())),
                ...
                ObjectRequirement::PatternPropertyRequirement { matcher, schema } => {
                    match matcher {
                        ...
                        PropertyNameMatcher::NamedProperty(name) => {
                            requirements.push(ObjectRequirementView::PatternPropertyRequirement {
                                pattern: format!("^{}$", escape_ecma262(name)),
                                schema: self.wrap_child(schema),
                            });
                        }
                    }
...
fn escape_ecma262(literal: &str) -> String {
...
fn string_view(leaf: &StringLeaf, ctx: &CanonicalizationContext) -> StringView {
    StringView {
        ...
        content: leaf
            ...
            .collect(),
        extended_regex: leaf.extended_regex(ctx),
    }
}
...
mod tests {
    ...
    #[test]
    fn named_property_requirement_pattern_is_ecma_valid() {
        ...
        let pattern = branches
            ...
            .expect("a pattern-property requirement branch");
    ...
    // The extended-pattern classification must honor the session's engine limits: a pattern past
    // the configured `size_limit` cannot compile on the standard engine under those options.
    #[test]
    fn extended_regex_classification_honors_pattern_size_limit() {
        let schema = json!({"type": "string", "pattern": "^(abc|def)+$"});
        let strict = crate::canonical::options()
            .with_pattern_options(&crate::PatternOptions::regex().size_limit(1))
            .canonicalize(&schema)
            .expect("canonicalize");
        let CanonicalView::String(StringView { extended_regex, .. }) = strict.view() else {
            panic!("expected String view");
        };
        assert!(
            extended_regex,
            "size-limited engine cannot compile the pattern"
        );
        let CanonicalView::String(StringView { extended_regex, .. }) = view_of(schema) else {
            panic!("expected String view");
        };
        assert!(!extended_regex, "default limits compile the pattern");
    }
