...
impl Validate for DependenciesValidator {
    fn is_valid(&self, instance: &Value, ctx: &mut LightweightContext) -> bool {
        if let Value::Object(item) = instance {
            for (property, node) in &self.dependencies {
                if item.contains_key(property) && !node.is_valid(instance, ctx.lightweight()) {
                    return false;
                }
            }
            true
        ...
        if let Value::Object(item) = instance {
            for (property, dependency) in &self.dependencies {
                if item.contains_key(property) {
                    dependency.validate(instance, location, ref_path, ctx)?;
                }
            }
        }
        Ok(())
    ...
    fn iter_errors<'i>(
        ...
        if let Value::Object(item) = instance {
            ...
            for (property, node) in &self.dependencies {
                if item.contains_key(property) {
                    errors.extend(node.iter_errors(instance, location, ref_path, ctx));
                }
            }
    ...
    fn evaluate(
        ...
        if let Value::Object(item) = instance {
            ...
            for (property, dependency) in &self.dependencies {
                if item.contains_key(property) {
                    children.push(dependency.evaluate_instance(instance, location, ref_path, ctx));
                }
            }
