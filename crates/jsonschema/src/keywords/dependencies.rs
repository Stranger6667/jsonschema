...
impl Validate for DependenciesValidator {
        ...
        if let Value::Object(item) = instance {
            for (property, dependency) in &self.dependencies {
                if item.contains_key(property) {
                    dependency.validate(instance, location, evaluation_path, ctx)?;
                }
            }
        }
        Ok(())
    ...
    fn iter_errors<'i>(
        ...
        if let Value::Object(item) = instance {
            ...
            for (property, node) in &self.dependencies {
                if item.contains_key(property) {
                    errors.extend(node.iter_errors(instance, location, evaluation_path, ctx));
                }
            }
    ...
    fn evaluate(
        ...
        if let Value::Object(item) = instance {
            ...
            for (property, dependency) in &self.dependencies {
                if item.contains_key(property) {
                    children.push(dependency.evaluate_instance(
                        instance,
                        location,
                        evaluation_path,
                        ctx,
