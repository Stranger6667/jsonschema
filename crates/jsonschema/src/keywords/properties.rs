        ...
        if let Value::Object(item) = instance {
            for (name, node) in &self.properties {
                if let Some(item) = item.get(name) {
                    node.validate(item, &location.push(name), ref_path, ctx)?;
                }
            }
        }
        Ok(())
        ...
        if let Value::Object(item) = instance {
            ...
            for (name, node) in &self.properties {
                if let Some(prop) = item.get(name) {
                    ...
                    errors.extend(node.iter_errors(prop, &instance_path, ref_path, ctx));
                }
            }
        ...
        if let Value::Object(props) = instance {
            ...
            for (prop_name, node) in &self.properties {
                if let Some(prop) = props.get(prop_name) {
                    ...
                    children.push(node.evaluate_instance(prop, &path, ref_path, ctx));
                }
            }
