...
impl Validate for ItemsArrayValidator {
    ...
    fn validate<'i>(
        ...
        if let Value::Array(items) = instance {
            for (idx, (item, node)) in items.iter().zip(self.items.iter()).enumerate() {
                node.validate(item, &location.push(idx), evaluation_path, ctx)?;
            }
        }
        Ok(())
    }
    ...
    fn iter_errors<'i>(
        ...
        if let Value::Array(items) = instance {
            ...
            for (idx, (item, node)) in items.iter().zip(self.items.iter()).enumerate() {
                errors.extend(node.iter_errors(item, &location.push(idx), evaluation_path, ctx));
            }
    ...
    fn validate<'i>(
        ...
        if let Value::Array(items) = instance {
            for (idx, item) in items.iter().enumerate() {
                self.node
                    .validate(item, &location.push(idx), evaluation_path, ctx)?;
            }
        }
        Ok(())
    }
