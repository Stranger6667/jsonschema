...
impl Validate for RefValidator {
    ...
    fn validate<'i>(
        ...
        // Create stack-local ref path entry - automatically "popped" when function returns
        let child_path = ref_path.push(&self.ref_location, &self.ref_target_base);
...
fn extract_ref_target_base(alias: &referencing::Uri<String>) -> Location {
    if let Some(fragment) = alias.fragment() {
        let fragment_str = fragment.as_str();
        if fragment_str.starts_with('/') {
            let mut location = Location::new();
            for segment in fragment_str.split('/').filter(|s| !s.is_empty()) {
                // Unescape JSON Pointer encoding (~0 → ~, ~1 → /) before join re-escapes
                location = location.join(unescape_segment(segment));
            }
            location
        } else {
            Location::new()
