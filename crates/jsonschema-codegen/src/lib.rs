...
///
/// The macro reads and compiles the schema at compile time and generates inherent
/// methods on the annotated struct:
///
...
// Keyword names the generated dispatch validates itself; a custom keyword
// under one of these names would run in addition to the built-in check
// instead of replacing it like the runtime validator does.
    ...
    if path.leading_colon.is_none() {
        ...
        if !is_crate_path {
            return Err(syn::Error::new_spanned(
                path,
                format!(
                    "Custom {entry_kind} paths must be absolute (`::...`) or start with `crate::`"
                ),
            ));
        }
    }
...
so the generated methods cannot depend on type parameters",
    ...
    // Validate the base URI first: a malformed one would otherwise surface as
    // an unspecific registry error.
