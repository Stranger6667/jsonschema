...
pub(crate) fn compile(ctx: &CompileContext<'_>, value: &Value) -> CompiledExpr {
    ...
    let err_expr = if let Some(f) = value.as_f64() {
        quote! { jsonschema::__private::error::multiple_of(#schema_path, __path.clone(), instance, #f) }
    } else {
        return CompiledExpr::from(check_ts);
    };
                ...
                quote! {
                    {
                        ...
                        jsonschema::__private::error::multiple_of(#schema_path, __path.clone(), instance, MOF.clone())
                    }
                }
            } else if let Some(f) = value.as_f64() {
                quote! { jsonschema::__private::error::multiple_of(#schema_path, __path.clone(), instance, serde_json::Value::from(#f)) }
            } else {
                return CompiledExpr::from(check_ts);
            }
        } else if let Some(f) = value.as_f64() {
...
fn generate_multiple_of_check(value: &Value) -> TokenStream {
    ...
    if let Some(multiple) = value.as_f64() {
        if multiple.fract() == 0.0 {
            quote! { jsonschema::__private::numeric::is_multiple_of_integer(n, #multiple) }
        } else {
            quote! { jsonschema::__private::numeric::is_multiple_of_float(n, #multiple) }
        }
    } else {
        quote! { true }
    }
