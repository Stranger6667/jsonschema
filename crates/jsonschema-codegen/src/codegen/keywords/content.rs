...
// Names and helper fns mirror the runtime validator's default content maps.
const BUILTIN_ENCODINGS: &[&str] = &["base64", "base64url", "base32", "base32hex", "base16"];

fn builtin_encoding_fns(name: &str) -> (TokenStream, TokenStream) {
    match name {
        "base64" => (
        ...
        _ => unreachable!("not a builtin encoding: {name}"),
    }
}
...
fn resolve_encoding(ctx: &CompileContext<'_>, name: &str) -> Option<(TokenStream, TokenStream)> {
    ...
    BUILTIN_ENCODINGS
        .contains(&name)
        .then(|| builtin_encoding_fns(name))
}
                        ...
                        Ok(Some(__decoded)) => {
                            if !#media_check(&__decoded) {
                                return Some(jsonschema::__private::error::content_media_type(
                                    #media_path, __path.clone(), instance, #media_name,
                                ));
                            }
                        }
                        Ok(None) => {
                            return Some(jsonschema::__private::error::content_encoding(
                                #encoding_path, __path.clone(), instance, #encoding_name,
                            ));
                        }
