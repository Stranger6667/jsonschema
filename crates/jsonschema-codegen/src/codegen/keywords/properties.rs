...
pub(crate) fn compile(
    ...
    // The single `obj.iter().all(...)` pass shared by `iter_check` and
    // `base_iter_check` below.
    ...
    let iter_check: CompiledExpr = match &main_iter_is_valid {
        None => CompiledExpr::always_true(),
        Some(is_valid_ts) => CompiledExpr::from_bool_expr(is_valid_ts.clone(), &schema_path),
    };

    let base_iter_check: CompiledExpr = if let Some(is_valid_ts_main) = main_iter_is_valid {
        let v = build_validate_block(
            ctx,
            &compiled_props,
            &tracked,
            &wildcard_arm_body,
        ...
        if tracked.is_empty() {
            CompiledExpr::with_validate_blocks(base_is_valid, v)
        } else {
    ...
    if tracked.is_empty() {
        ...
        if known_keys_precheck.is_trivially_true() {
            ...
            // We must emit that check in validate BEFORE the required-field check so that
            // additionalProperties errors are reported before missing-required errors, matching
