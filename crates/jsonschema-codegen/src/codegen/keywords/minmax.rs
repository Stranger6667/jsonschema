...
pub(crate) fn generate_numeric_check(op: ComparisonOp, limit: &Value) -> TokenStream {
    ...
    if let Some(u) = limit.as_u64() {
        quote! { jsonschema::__private::numeric::#cmp_fn(n, #u as u64) }
    } else if let Some(i) = limit.as_i64() {
        quote! { jsonschema::__private::numeric::#cmp_fn(n, #i as i64) }
    } else if let Some(f) = limit.as_f64() {
        quote! { jsonschema::__private::numeric::#cmp_fn(n, #f as f64) }
    } else {
        quote! { true }
    }
}
