...
/// Compile a user-registered keyword into a lazily constructed `Keyword` instance.
///
/// The factory runs at first use of the validator; a failing factory panics
/// there, since the schema is fixed at compile time and the failure is
/// deterministic (the runtime analogue is a `build()` error).
