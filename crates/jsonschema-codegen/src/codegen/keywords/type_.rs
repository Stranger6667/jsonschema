        ...
        Value::Array(types) => {
            ...
            if has_integer || has_number {
                let number_arm = if has_number {
                    ...
                    let int_check = crate::codegen::emit_serde::integer_number_guard(ctx.draft);
                ...
                for ty in &type_names {
                    let arm = match *ty {
                        "string" => {
                            let pattern = crate::codegen::emit_serde::pattern_string();
                            quote! { #pattern => true }
                        }
                        "boolean" => {
                            let pattern = crate::codegen::emit_serde::pattern_boolean();
                            quote! { #pattern => true }
                        }
                        "null" => {
                            let pattern = crate::codegen::emit_serde::pattern_null();
                            quote! { #pattern => true }
                        }
                        "array" => {
                            let pattern = crate::codegen::emit_serde::pattern_array();
                            quote! { #pattern => true }
                        }
                        "object" => {
                            let pattern = crate::codegen::emit_serde::pattern_object();
                            quote! { #pattern => true }
                        }
                        _ => continue,
                    };
                ...
                let patterns: Vec<TokenStream> = type_names
                    ...
                    .filter_map(|ty| match *ty {
                        "string" => Some(crate::codegen::emit_serde::pattern_string()),
                        "boolean" => Some(crate::codegen::emit_serde::pattern_boolean()),
                        "null" => Some(crate::codegen::emit_serde::pattern_null()),
                        "array" => Some(crate::codegen::emit_serde::pattern_array()),
                        "object" => Some(crate::codegen::emit_serde::pattern_object()),
                        _ => None,
                    })
                    .collect();
                wrap_type_check(
