...
use super::{
    BackendAccessors, BackendMatchArms, BackendPatterns, BackendSymbols, BackendTypeChecks,
};
...
impl BackendAccessors for SerdeJsonBackend {
    ...
    fn instance_object_property_as_i64(&self, key: &str) -> TokenStream {
        quote! {
            match instance {
                serde_json::Value::Object(obj) => obj.get(#key).and_then(serde_json::Value::as_i64),
                _ => None,
            }
        }
    }
