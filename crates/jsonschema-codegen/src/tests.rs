...
mod regression_tests {
    ...
    fn duplicate_custom_format_entries_fail_parsing() {
        ...
        assert_eq!(err.to_string(), "Duplicate format entry: `currency`");
    }

    #[test]
    fn duplicate_custom_keyword_entries_fail_parsing() {
        let parsed = syn::parse2::<crate::Config>(quote! {
            schema = r#"{"type":"string"}"#,
            keywords = {
                "even" => crate::keywords::even_a,
                "even" => crate::keywords::even_b,
            }
        });
        let Err(err) = parsed else {
            panic!("duplicate keyword entries should be rejected");
        };
        assert_eq!(err.to_string(), "Duplicate keyword entry: `even`");
    }
    ...
    #[test_case(
        ...
        "Duplicate attribute: `schema`";
        "duplicate_schema"
    )]
    fn conflicting_schema_source_entries_fail_parsing(input: TokenStream, expected: &str) {
...
mod determinism_tests {
    ...
    // A schema with multiple distinct $ref targets exercises the helper-name
    // allocation and the iteration of `Helpers.bodies` / `validate_iter_bodies`
    // during emission. If those maps used a HashMap, emitted helper-function
    // order would vary across runs.
    #[test]
    ...
    // Two candidate discriminator keys with identical stats: tie-breaking must
...
mod emission_size_tests {
    ...
    // Branch validity guards inside unevaluated* evaluation must be shared
    // helper fns, not re-inlined per nesting level: inline guards make emitted
    // code grow quadratically with applicator nesting depth.
    #[test]
