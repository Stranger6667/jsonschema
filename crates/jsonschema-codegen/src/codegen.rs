...
impl<'a> Codegen<'a> {
    ...
    fn compile_const(&self, value: &Value) -> TokenStream {
        ...
        // TODO: maybe precompile hash instead???
        // TODO: precompile item type for fast rejection?
        quote! {
            {
                ...
                // Use jsonschema-aware comparison for numeric equality (0 == 0.0)
    ...
    fn compile_one_of(&self, value: &Value) -> TokenStream {
        if let Some(schemas) = value.as_array() {
            ...
            let checks = schemas.iter().map(|schema| {
                let validation = self.compile_schema(schema);
                quote! {
                    if #validation {
                        if matched {
