...
impl Registry {
    ...
    fn try_from_resources_impl(
        ...
        validate_custom_metaschemas(&resources)?;
            ...
            if !resources.contains_key(&fragmentless) {
                let retrieved = match retriever.retrieve(&fragmentless) {
                    ...
                    Err(error) => {
                        ...
                        // $schema fetch failures during resource processing are non-fatal here,
                        // but unregistered custom meta-schemas will be caught in validate_custom_metaschemas()
                        match kind {
...
fn validate_custom_metaschemas(resources: &ResourceMap) -> Result<(), Error> {
    for resource in resources.values() {
        if resource.draft() == Draft::Unknown {
                ...
                .and_then(|s| s.as_str())
            {
                ...
                let normalized_schema_uri = schema_uri.trim_end_matches('#');
                match uri::from_str(normalized_schema_uri) {
                    Ok(meta_uri) => {
...
/// Check if a schema URI is a standard JSON Schema meta-schema.
fn is_standard_metaschema(schema_uri: &str) -> bool {
    schema_uri.starts_with("https://json-schema.org/draft/")
        || schema_uri.starts_with("http://json-schema.org/draft-")
}
        ...
        if object.len() < 3 {
            for (key, value) in object {
                if key == "$ref" {
                    ...
                    // $schema indicates which meta-schema this schema conforms to
                    // For custom meta-schemas, we want to fetch them for vocabulary resolution
                    // but they're not required to succeed (schema compilation should work even if meta-schema is unavailable)
                    if let Some(reference) = value.as_str() {
                        if is_standard_metaschema(reference) {
                            // Track that this schema refers to a known meta-schema
                            *refers_metaschemas = true;
                        } else {
                            // Custom meta-schema - try to fetch it for vocabulary resolution
                            // Use on_reference! to add it to external resources
...
mod tests {
    ...
    fn test_try_with_resources_requires_registered_custom_meta_schema() {
        ...
        assert!(error
            .to_string()
            .contains("Unknown meta-schema: 'http://example.com/meta/custom'"));
    }
    ...
    fn test_chained_custom_meta_schemas() {
        ...
        let meta_schema_b = Resource::from_contents(json!({
            "$id": "http://example.com/meta/level-b",
            "$schema": "https://json-schema.org/draft/2020-12/schema",
            "$vocabulary": {
        ...
        let meta_schema_a = Resource::from_contents(json!({
            "$id": "http://example.com/meta/level-a",
            "$schema": "http://example.com/meta/level-b",
        ...
        let schema = Resource::from_contents(json!({
            "$id": "http://example.com/schemas/my-schema",
            "$schema": "http://example.com/meta/level-a",
        ...
        Registry::try_from_resources([
            ("http://example.com/meta/level-b", meta_schema_b),
            ("http://example.com/meta/level-a", meta_schema_a),
            ("http://example.com/schemas/my-schema", schema),
        ])
