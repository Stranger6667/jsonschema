...
pub enum Draft {
    ...
    #[default]
    Draft202012,
    /// Unknown/custom meta-schema (to be resolved during registry initialization)
    Unknown,
}
    ...
    pub fn detect(self, contents: &Value) -> Draft {
            ...
            .and_then(|schema| schema.as_str())
        {
            match schema.trim_end_matches('#') {
                ...
                // For custom/unknown meta-schemas, return Unknown for now
                // Validation happens during registry building
                _value => Draft::Unknown,
            }
        } else {
            self
        }
