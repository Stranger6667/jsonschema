...
///
/// Navigation never recurses; reporting does. `into_parts` hands the instance to the caller, so
/// the depth also has to be one their `Clone`/`Display`/`Drop` survives, and none of those is
/// ours to make iterative. Worst measured case - debug build, nested objects, 2 MiB thread -
/// is `Clone`, fine at 1000 and aborting by 1500.
pub const MATERIALIZATION_NESTING_LIMIT: usize = 512;
...
impl<'a> Node<'a, Jsonb> for JsonbNode<'a> {
    ...
    // A `const` built in code carries no parser depth cap, so this walks rather than recurses.
        ...
        while let Some((node, expected)) = pending.pop() {
            let equal = match expected {
                ...
                Value::Object(map) => node.as_object().is_some_and(|object| {
                    ...
                    // A stored key that is not UTF-8 matches no key of a `serde_json` object,
                    // whose keys are all `String`.
...
/// [`equal_nodes`] with the caller's worklist, so a run of comparisons allocates once.
    ...
    fn is_unique(&self) -> bool {
        ...
        crate::unique::is_unique_by(
            ...
            |left, right| {
                ...
                pending.clear();
                equal_nodes_into(*left, *right, &mut pending)
            },
