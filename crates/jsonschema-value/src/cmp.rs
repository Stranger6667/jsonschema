...
pub fn equal_numbers<L: crate::JsonNumber>(left: &L, right: &serde_json::Number) -> bool {
    ...
    #[cfg(not(feature = "arbitrary-precision"))]
    {
        ...
        // The schema's number was already rounded into an `f64` when it was parsed, so no
        // comparison here can be exact. `uniqueItems`, weighing two instance values, still is.
    }
}
