...
use jsonschema_value::{
    cmp, jsonb::take_pending_error, types::JsonType, unique, Array, Json, JsonNumber, Jsonb,
    JsonbNode, LazyInstance, Node, Object,
};
...
#[test_case("9999.999999999999", None, None, 9_999.999_999_999_998, false; "mantissa past 2^53")]
fn numeric_accessors(
    ...
    let encoded = encode_numeric_text_scalar(text, NumericForm::Long);
...
fn numeric_header_forms_agree(form: NumericForm) {
    let fraction_encoded = encode_numeric_text_scalar("1.5", form);
    let fraction = Jsonb::root(&fraction_encoded).as_number().expect("number");
    let integer_encoded = encode_numeric_text_scalar("42", form);
...
fn wide_numbers_keep_their_digits() {
    let encoded = encode_numeric_text_scalar("1e100", NumericForm::Long);
...
fn equals_value_number_beyond_f64(expected: &Value, want: bool) {
    let encoded = encode_numeric_text_scalar("1e400", NumericForm::Long);
...
#[test_case(json!({"a": 1}), &json!({"b": 1}), false; "expected has another key")]
#[test_case(json!(true), &json!(true), true; "same boolean")]
#[test_case(json!(true), &json!(false), false; "different boolean")]
#[allow(clippy::needless_pass_by_value)]
...
#[test_case(json!(0); "same integer")]
#[test_case(json!(0.0); "zero written as a float")]
#[test_case(json!(1.0); "integer written as a float")]
#[test_case(json!(-1); "negative")]
#[allow(clippy::needless_pass_by_value)]
fn is_unique_large_array_finds_a_duplicate(duplicate: Value) {
...
// Beyond `f64` the digits cannot survive into a `serde_json::Number`.
#[cfg(not(feature = "arbitrary-precision"))]
#[test_case("1e400", false; "positive")]
#[test_case("-1e400", true; "negative")]
fn numbers_past_f64_record_an_error(text: &str, negative: bool) {
    let encoded = encode_numeric_text_scalar(text, NumericForm::Long);
    let node = Jsonb::root(&encoded);
    let reported = node.as_number().expect("number").to_number().to_string();
    assert_eq!(reported.starts_with('-'), negative, "sign of {reported}");
    assert_eq!(take_pending_error().as_deref(), Some("Number out of range"));
}
...
// `serde_json`'s own parser depth.
const REPORTING_DEPTH: usize = 128;

fn wrapped(mut value: Value, depth: usize) -> Value {
    for _ in 0..depth {
...
fn report(node: JsonbNode<'_>, lazy: bool) -> Value {
    if lazy {
        node.lazy_value().get().clone().into_owned()
    } else {
        node.to_value().into_owned()
    }
}

#[test_case(false; "to_value")]
#[test_case(true; "lazy value")]
fn reporting_past_the_depth_limit_records_an_error(lazy: bool) {
    let encoded = encode_nested_arrays(REPORTING_DEPTH);
    let value = report(Jsonb::root(&encoded), lazy);
    assert_eq!(value, wrapped(Value::Null, REPORTING_DEPTH));
    assert_eq!(
...
#[test_case(false; "to_value")]
#[test_case(true; "lazy value")]
fn reporting_within_the_depth_limit_is_exact(lazy: bool) {
    let encoded = encode_nested_arrays(REPORTING_DEPTH - 1);
    let value = report(Jsonb::root(&encoded), lazy);
    assert_eq!(value, wrapped(json!([]), REPORTING_DEPTH - 1));
    assert_eq!(take_pending_error(), None);
}

#[hegel::test(test_cases = 5_000)]
fn as_f64_matches_the_text(tc: TestCase) {
    let value: f64 = tc.draw(gs::floats().allow_nan(false).allow_infinity(false));
    let text = value.to_string();
    let encoded = encode_numeric_text_scalar(&text, NumericForm::Long);
    let number = Jsonb::root(&encoded).as_number().expect("number");
    assert_eq!(number.as_f64(), text.parse().ok(), "{text}");
}
...
#[test]
fn non_utf8_key_matches_no_expected_key() {
    let encoded = encode_raw_key_object(b"\xff");
    assert!(!Jsonb::root(&encoded).equals_value(&json!({"\u{FFFD}": null})));
}

#[test]
fn invalid_utf8_string_reads_lossily() {
    let bytes: &[u8] = b"a\xff\xfe";
    let encoded = assemble(JB_FARRAY | JB_FSCALAR | 1, &[bytes.len() as u32], bytes);
    let node = Jsonb::root(&encoded);
    assert_eq!(node.as_string().as_deref(), Some("a\u{FFFD}\u{FFFD}"));
    assert_eq!(node.string_length(), Some(3));
}

#[test]
fn non_string_has_no_string() {
    let encoded = encode(&json!(1));
    assert_eq!(Jsonb::root(&encoded).as_string(), None);
}

#[test_case("1", true; "integer")]
#[test_case("1.0", false; "trailing zero")]
#[test_case("1.5", false; "fraction")]
fn is_written_as_integer(text: &str, expected: bool) {
    let encoded = encode_numeric_text_scalar(text, NumericForm::Long);
    let number = Jsonb::root(&encoded).as_number().expect("number");
    assert_eq!(number.is_written_as_integer(), expected);
}

#[test_case(&json!([]), true; "empty")]
#[test_case(&json!([1]), true; "one element")]
#[test_case(&json!([1, -1]), true; "opposite signs")]
#[test_case(&json!([true, true]), false; "same booleans")]
#[test_case(&json!([[1], [2]]), true; "arrays differ in an element")]
#[test_case(&json!([[1], [1, 2]]), true; "arrays differ in length")]
#[test_case(&json!([{"a": 1}, {"b": 1}]), true; "objects differ in a key")]
#[test_case(&json!([{"a": 1}, {"a": 1, "b": 2}]), true; "objects differ in size")]
fn is_unique_small_arrays(instance: &Value, expected: bool) {
    let encoded = encode(instance);
    let array = Jsonb::root(&encoded).as_array().expect("array");
    assert_eq!(array.is_unique(), expected);
}

// Drawn from a small pool so duplicates are common, and long enough to reach hashing.
#[hegel::test(test_cases = 2_000)]
fn is_unique_agrees_with_serde_json(tc: TestCase) {
    let pool: Vec<Value> = tc.draw(gs::vecs(json_gs::values()).min_size(1).max_size(6));
    #[cfg(not(feature = "arbitrary-precision"))]
    let pool: Vec<Value> = pool.iter().map(unambiguous_numbers).collect();
    let picks: Vec<usize> =
        tc.draw(gs::vecs(gs::integers::<usize>().max_value(pool.len() - 1)).max_size(40));
    let items: Vec<Value> = picks.iter().map(|&index| pool[index].clone()).collect();
    let encoded = encode(&Value::Array(items.clone()));
    let array = Jsonb::root(&encoded).as_array().expect("array");
    assert_eq!(array.is_unique(), unique::is_unique(&items), "{items:?}");
}
