...
#[should_panic(expected = "JSONB value exceeds maximum materialization nesting depth")]
fn materializing_beyond_the_nesting_limit_panics(lazy: bool) {
    let encoded = encode_nested_arrays(MATERIALIZATION_NESTING_LIMIT);
    let node = Jsonb::root(&encoded);
    if lazy {
...
fn materializing_at_the_nesting_limit_succeeds(lazy: bool) {
    let encoded = encode_nested_arrays(MATERIALIZATION_NESTING_LIMIT - 1);
    let node = Jsonb::root(&encoded);
    if lazy {
        let value = node.lazy_value();
        let _ = value.get();
