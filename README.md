...
- ⚡ Compile-time validators for Rust, Python and Ruby extension modules
...
- **75-692x** faster than `valico` and **10-78x** faster than `jsonschema_valid` for complex schemas (**>1,000,000x** on FHIR)
- Generally **2.8-40x** faster than `boon`, **>280x** on FHIR, and **>5000x** for recursive schemas

When the schema is known at build time, the `#[jsonschema::validator]` macro (`macros` feature) generates a specialized validator at compile time. This avoids schema compilation at runtime and validates roughly **2.3-10x** faster than the runtime validator across our benchmark schemas.
